...
import flowDefinition from '@site/src/data/flow-definitions/design-patterns/drain-internal-channels.json';
...
Use this pattern when one branch of a Flow publishes work for another branch. **Init** starts **Main** and **Side** in parallel. **Main** publishes each document command to **SideStepData**. **Side** waits for one Channel value, writes it, and waits again.

The key is the sentinel published by **Finalize**. After **Main** has published every ordinary message, **Finalize** publishes a final command to **SideStepData**. Because a Channel is a FIFO queue, **Side** cannot receive that sentinel until it has received every earlier message. It completes only when it receives the sentinel, so the Flow does not finish before every published document is handled. With several producer branches, make **Finalize** wait for all producers before it publishes the sentinel.
