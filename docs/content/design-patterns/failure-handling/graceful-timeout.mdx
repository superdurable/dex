...
**FlowGracefulTimeout** starts **LongWait**. When the Flow deadline expires, Dex runs **handleTimeout**. The handler response uses Execute semantics.

<DocsFlowDefinitionGraph graph={flowDefinition} />

## Core implementation

The complete Flow definition registers **LongWait** and the timeout hook. Start the Flow with a positive timeout; **HANDLER** is explicit below and is also the default when the Flow provides the hook. The example gives the hook its own attempt timeout and retry policy with **FlowTimeoutHandlerOptions**.
...
func (longWait) WaitFor(_ dex.Context, successful bool) (*dex.Wait, error) {
	if successful {
...
func (longWait) Execute(_ dex.Context, _ bool) (*dex.StepDecision, error) {
...
final class FlowGracefulTimeout implements Flow<Boolean> {
    private final LongWait longWait = new LongWait();

    @Override
    public StepList<Boolean> getSteps() {
        return StepList.startStep(longWait);
    }
...
class FlowGracefulTimeout implements Flow<boolean> {
  private readonly longWait = new LongWait();
  ...
  public getSteps() {
    return StepList.startStep(this.longWait);
  }
