...
```python
await client.start_flow(flow, flow_id, successful, StartFlowOptions(
...
```go
timeout := time.Minute
_, err := client.StartFlow(ctx, flow, flowID, successful, dex.StartFlowOptions{
	Timeout: &timeout, TimeoutPolicy: dex.TimeoutHandler,
})
```
...
final class FlowGracefulTimeout implements Flow<Boolean> {
    ...
    @Override public StepList<Boolean> getSteps() {
    ...
    final class LongWaitStep implements Step<Boolean> {
        ...
        @Override public Wait waitFor(final Context context, final Boolean successful) {
            return Boolean.TRUE.equals(successful)
                    ? Wait.skipImmediately()
                    : Wait.until(Timer.byDuration(Duration.ofSeconds(65)));
        }
        @Override public StepDecision execute(final Context context, final Boolean successful) {
            return StepDecision.forceComplete("Workflow completed successfully");
        }
    }

    @Override public StepDecision handleTimeout(final Context context) {
...
```java
client.startFlow(flowGracefulTimeout, workflowId, successfulWorkflow,
        StartFlowOptions.newBuilder().timeout(Duration.ofMinutes(1))
                .timeoutPolicy(FlowTimeoutPolicy.HANDLER).build());
```
...
class LongWaitStep implements Step<boolean> {
  ...
  public waitFor(_context: Context, successful: boolean): Wait {
    return successful ? Wait.skipImmediately() : Wait.until(Timer.byDuration(65_000));
  }
...
class FlowGracefulTimeout implements Flow<boolean> {
  ...
  public getFlowType(): string { return "FlowGracefulTimeout"; }
  public getSteps() { return StepList.startStep(this.longWaitStep); }
...
await client.startFlow(flowGracefulTimeout, workflowId, successful, {
  timeoutMs: 60_000, timeoutPolicy: FlowTimeoutPolicy.HANDLER,
});
```
...
```rust
#[derive(Default)]
pub struct FlowGracefulTimeout { long_wait_step: LongWaitStep }

impl Flow for FlowGracefulTimeout {
    type StartInput = bool;
    fn steps(&self) -> StepList<'_, Self::StartInput> { StepList::start(&self.long_wait_step) }
    fn timeout_handler(&self) -> Option<FlowTimeoutHandler<Self>> { Some(Self::handle_timeout) }
}

impl Step for LongWaitStep {
    type Input = bool;
    fn wait_for(&self, _context: &mut Context, successful: bool) -> HandlerResult<Wait> {
        Ok(Wait::until(Timer::by_duration(Duration::from_secs(if successful { 1 } else { 65 }))))
    }
...
```rust
client.start_flow_with_options(&flow, &flow_id, successful,
    StartFlowOptions::new().timeout(Duration::from_secs(60))
        .timeout_policy(FlowTimeoutPolicy::Handler))?;
```
