...
**ManualRecoveryFlow** starts **DoWork**. Its failure policy makes three exponential-backoff retries, then proceeds to **ManualStep**. **ManualStep** waits for either the **Retry** Channel or the **Skip** Channel. Retry returns to **DoWork** after the operator resolves the problem; skip fails the Flow with an explicit reason.

## Core implementation

**DoWork** owns the retry policy and its exhausted-failure movement. **ManualStep** owns the durable wait for the operator's Channel message and turns that message into the next decision.
...
func (doWork) Execute(_ dex.Context, shouldFail bool) (*dex.StepDecision, error) {
	if shouldFail {
