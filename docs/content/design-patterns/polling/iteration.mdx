...
final class IterationStep implements Step<String> {
    ...
    @Override public StepDecision execute(final Context context, final String pageToken) {
        final String nextPageToken = pageToken.isEmpty() ? "page-2" : pageToken.equals("page-2") ? "page-3" : "";
        return nextPageToken.isEmpty() ? StepDecision.gracefulComplete() : StepDecision.goTo(IterationStep.class, nextPageToken);
    }
}
```
...
impl Step for IterationStep {
    type Input = String;
    fn step_type(&self) -> &'static str { "IterationStep" }
    fn execute(&self, _: &mut Context, page_token: String) -> HandlerResult<StepDecision> {
        let next_page_token = match page_token.as_str() { "" => "page-2", "page-2" => "page-3", _ => "" };
        if next_page_token.is_empty() { Ok(StepDecision::graceful_complete(())) } else { Ok(StepDecision::go_to(&IterationStep, next_page_token.to_owned())) }
    }
}
```
