...
import flowDefinition from '@site/src/data/flow-definitions/design-patterns/parallel-await.json';
...
**Init** starts **DoWork** executions and **AwaitStep** together. Each worker sleeps for a random short duration, then publishes one message to **CompleteCh**. **AwaitStep** uses **WaitFor** for the required number of messages, then completes only after every branch has finished. Channel messages are durable, so completions published before **AwaitStep** begins waiting still count.
