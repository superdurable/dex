...
import flowDefinition from '@site/src/data/flow-definitions/design-patterns/parallel-dynamic.json';
...
**Init** builds one **DoWork** movement per input item. Dex schedules those movements concurrently. Each worker sleeps for a random short duration to make the concurrent completion order visible. Use this pattern when the degree of parallelism is known only at runtime.
