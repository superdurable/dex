...
![Back pressure for high availability](/img/design-patterns/parallel-subflows-back-pressure.png)

The client starts **SubmitRequestFlow** instead of retrying an RPC in memory. **SubmitStep** partitions the request, calls the selected parent's **SendRequest** RPC, and completes only after the parent accepts it. A false response fails the Step, so the normal Step retry policy durably backs off and tries again. If the submitting process or Worker restarts, Dex preserves the retry.
