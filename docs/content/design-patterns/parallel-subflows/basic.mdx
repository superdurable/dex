...
Start one SubFlow for each input and wait for every SubFlow to complete.

<DocsFlowDefinitionGraph graph={flowDefinition} />

**SubFlowsStep.WaitFor** builds one SubFlow condition per request and combines them with **allOf**. The parent resumes only after all children finish. This is the smallest useful parent–child fan-out when partial completion and cancellation are not required.
...
- [Wait for half](/design-patterns/parallel-subflows/wait-for-half)
- [Parallel SubFlows](/design-patterns/parallel-subflows)
