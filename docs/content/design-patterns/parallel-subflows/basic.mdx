---
title: Basic SubFlow Fan-out
---

# Basic SubFlow Fan-out

Start one SubFlow for each input, wait until half complete, then stop the unfinished SubFlows.

![Basic SubFlow fan-out](/img/design-patterns/parallel-subflows-basic.png)

**SubFlowsStep.WaitFor** starts every **ExampleSubFlow** and expresses “any half” as an **anyCombinationOf** wait. When Dex invokes **Execute**, every SubFlow result is still addressable by index. Completed winners have a terminal status; unfinished losers have **RUNNING** status. The Step uses **SubFlowID** to cancel only those losers.
