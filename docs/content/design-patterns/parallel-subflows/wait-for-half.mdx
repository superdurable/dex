...
Each child runs in its own **SubFlowStep** execution. A completed child publishes one message to **SubFlowCompletedCh**. **WaitSubFlows** waits for half of those messages, then publishes one **AllDoneCh** message for every possible unfinished child.
...
- [Basic parent Flow](/design-patterns/parallel-subflows/basic)
- [Long-lived parent](/design-patterns/parallel-subflows/long-lived-parent)
