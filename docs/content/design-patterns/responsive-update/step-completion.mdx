...
Leave **RequestTimeout** at 0 for an ordinary durable wait. Set it explicitly at an admission boundary; the controller below uses 10 seconds. **InternalHandlerTimeout** is an advanced Update-generation control and normally remains 0. See [Choose the timeout boundary](/design-patterns/responsive-update#choose-the-timeout-boundary) for details.
...
func (controller *controller) start(request *gin.Context) {
	...
	err = controller.client.WaitForStepCompletion(
		request.Request.Context(), flowID, sdk.StepExecutionID{StepType: "PersistData"},
