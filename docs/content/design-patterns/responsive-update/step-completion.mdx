...
The **WaitForStateCompletionFlow** sample persists a record in **PersistData**, then starts **BackgroundWork**. Its controller waits for **PersistData** before querying the persisted record. Choose a Step only after it has committed the state that makes the response valid.

## Core implementation

<SdkTabs examples={{python: 'examples/python/dex_examples/patterns/wait-for-state-completion/controller.py', go: 'examples/go/patterns/wait-for-state-completion/controller.go', java: 'examples/java/src/main/java/io/superdurable/dex/patterns/waitforstatecompletion/WaitForStateCompletionController.java', typescript: 'examples/typescript/src/patterns/wait-for-state-completion/controller.ts', rust: 'examples/rust/src/patterns/wait_for_state_completion/controller.rs'}}>
<SdkSnippet lang="go">
