...
Leave **RequestTimeout** at 0 for an ordinary durable wait. Set it explicitly at an admission boundary. The runnable controller below remains unbounded until its published SDK dependency includes this API. Set **InternalHandlerTimeout** only to reclaim abandoned accepted handlers that threaten Temporal's in-flight Update limit. See [Choose the timeout boundary](/design-patterns/responsive-update#choose-the-timeout-boundary) for details.
