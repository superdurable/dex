...
Leave **RequestTimeout** at 0 for an ordinary durable wait. Set it explicitly at an admission boundary. The runnable controller below remains unbounded until its published SDK dependency includes this API. **InternalHandlerTimeout** is an advanced Update-generation control and normally remains 0. See [Choose the timeout boundary](/design-patterns/responsive-update#choose-the-timeout-boundary) for details.
