...
The **EngagementFlow** sample writes **EmployerId** from its start request. Its start endpoint waits until that durable identifier is available, then returns the Flow and run identifiers. The Flow remains active so a caller can later accept, decline, or opt out.
