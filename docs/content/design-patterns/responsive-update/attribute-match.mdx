...
Leave **MaximumWaitTime** at 0 for normal use and bound individual responses with the caller deadline. Positive values are rare because short budgets can add many Temporal Update events to Workflow history. If a positive value is necessary, prefer at least one minute. It returns a typed timeout to the caller. See [Choose the wait budget](/design-patterns/responsive-update#choose-the-wait-budget) for details.
