...
Dex derives the Request ID from the complete Attribute condition. Repeating the same condition on one active Flow shares one accepted durable Update, including across Continue-As-New. Leave **MaximumWaitTime** at 0 for normal use and bound individual responses with the caller deadline. Positive values are rare because short budgets can repeatedly create new -N Update generations and many Temporal Update history events. If a positive value is necessary, prefer at least one minute. It returns a typed timeout to the caller, but does not schedule a durable timer or immediately release the accepted handler. The handler observes its deadline on a later Workflow Task. Calls reattach to it until it finishes; only then can Dex create a new generation. See [Choose the durable handler lifetime](/design-patterns/responsive-update#choose-the-durable-handler-lifetime) for details.
