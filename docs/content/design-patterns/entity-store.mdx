...
as three Attributes, and opts those definitions into Attribute Store sync. The
...
2. **Synced Attribute definitions**: `display_name`, `email`, and
   `marketing_opt_in` project every initial value, RPC update, and deletion.
...
- `POST /design-pattern/entity-store/profile` — create a profile Flow
- `POST /design-pattern/entity-store/profile/update` — update its synced fields
- `GET /design-pattern/entity-store/profile?userId=...` — read authoritative Flow state
- `POST /design-pattern/entity-store/profile/clear?userId=...` — delete the synced values
