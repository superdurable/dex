...
**InterruptibleFlow** starts **WorkA** and **WorkB** in parallel. Each Step checks the durable **interruptSignal** before scheduling its next unit of work. The **interrupt** RPC sets that signal, letting either branch finish the Flow gracefully.

<DocsFlowDefinitionGraph graph={flowDefinition} />

## Core implementation

Each sample shows **WorkA**. It waits between units of work, completes when the interrupt signal arrives, and otherwise schedules its next loop iteration. **WorkB** follows the same pattern. The runnable examples also include Flow registration and the **interrupt** RPC.
