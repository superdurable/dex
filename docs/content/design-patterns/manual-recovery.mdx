...
**DoWorkStep** receives a Boolean input. A true input fails immediately; a false input completes the Flow. Its failure policy makes three exponential-backoff retries, then proceeds to **ManualStep**. The four total attempts include the initial execution.

**ManualStep** waits for either the **Retry Channel** or the **Skip Channel**. A person publishes **Retry Channel** after resolving the problem, and the sample runs **DoWorkStep** with false input. Publishing **Skip Channel** fails the Flow with a clear reason.

## Core implementation

Each sample defines the failure policy and the manual decision. The runnable example also registers the Flow and exposes its HTTP start endpoint.
