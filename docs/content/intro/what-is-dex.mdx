...
Dex is a **Durable Execution** engine. You write a **Flow** as ordinary application objects: **Steps**, **Attributes**, **Channels**, **Timers**, and **RPCs**. A **Worker** hosts your Step and RPC handlers as normal code. Dex Server keeps the control-flow decisions durable.

A **Step** can **WaitFor** (Channel, Timer, or both) and then **Execute**. Execute returns the next Step, completes, fails, or loops. **Attribute** holds durable data. **Channel** carries messages into a running Flow. **Timer** is a durable deadline. **RPC** lets outside code read or write while the Flow is running.

If you have not read [What is Durable Execution?](/intro/what-is-durable-execution), this page uses the order-processing example from there.

## Order processing as a Flow

Same flow: charge, wait for seller approval (with reminders), ship, refund if ship retries exhaust.
...
- The storefront uses the Client **wait-for-Step-completion** API on ChargeStep — not a homemade status poll.
- **Attribute `order-status`** is indexed so support can search stuck orders.

Runnable code: [`examples/go/products/order-processing`](https://github.com/superdurable/dex/tree/main/examples/go/products/order-processing), plus the same product under `examples/java`, `examples/python`, `examples/typescript`, and `examples/rust`. HTTP prefix `/products/order-processing/...`. Try it from the [examples playground](https://github.com/superdurable/dex/tree/main/examples/playground) (`id: order-processing`).
...
Frontend wait for “payment received”:

```python
await client.wait_for_step_completion(flow_id, StepExecutionId("ChargeStep"), timeout)
```

That Client API is also on the Go, Java, TypeScript, and Rust SDKs. The example HTTP route is `GET /products/order-processing/wait-charged?workflowId=...`.
...
Traditional Durable Execution runs workflow functions by **replaying** event history. Side effects go in **activities**. The workflow code must stay **deterministic**: no extra clocks, random, or IO in the replayed function.

Dex does not ask you to write a replayable workflow function. You declare Steps and movements. Worker handlers are ordinary code. Waits, RPC, and locking are part of the Flow model, not something you reconstruct from history primitives.
