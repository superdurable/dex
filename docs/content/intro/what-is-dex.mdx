...
Dex is a **Durable Execution** platform.

Dex is structural programming with only a few concepts as durable primitives. You use Dex to write a **Flow** filled with ordinary code: durable **Steps**, **Attributes**, **RPCs**, and durable conditions using **Channels** and **Timers**. Then you run **Workers** hosting your Flow. The **Client** calls **Dex Server** to start and interact with Flow instances. Dex Server dispatches Step and RPC invocation tasks to your Workers.
