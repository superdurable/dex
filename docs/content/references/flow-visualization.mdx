...
Version 2.0 recognizes operation-specific Connector factories such as **openai.NewCreateResponseStep** through the config's canonical Connector SDK marker and generated field metadata, rather than a list of connector or function names. Dex Web marks these Steps with a Connector icon. The generic SDK **MustNewQueryStep** and **MustNewMutationStep** escape hatches remain supported. Put the factory call directly in **DefineStep** or **DefineStartStep** and use an inline config literal. Step type, presentation, named **GoTo** branch targets (or generic **GoToBranch** targets), **StepRef** names, Result Attributes, Streams, and **ProceedToOnExecuteFailure** targets must be statically visible. Factory presentation supplies the Step group and explanation, so the generated Step does not need local directives. The graph emits one labeled transition per Connector branch, a write relation for the Result Attribute, and best-effort progress relations for structured and text Streams. Dynamic config, branch slices, targets, or resource references produce blocking diagnostics. See the [Connector factory example](https://github.com/superdurable/dex/blob/main/examples/go/products/connector-factory/workflow.go).
