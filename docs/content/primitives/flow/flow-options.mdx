...
**CronSchedule** start new executions on a schedule. Retry recovers one failed execution but Cron creates later executions. The backend decides what happens to overlapping or missed runs.

### StartDelay {#start-delay}

**StartDelay** accepts the Flow to start now, but waits before the **StartStep** can run. Use it for a one-off delay, for example, wait five minutes before processing a new order. Use **CronSchedule** for recurring schedule.
