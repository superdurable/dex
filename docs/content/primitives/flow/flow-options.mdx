...
Dex uses **HANDLER** by default when the Flow has a timeout handler. Otherwise it uses **FAIL**. **HANDLER** is invalid when the Flow has no handler. A retry starts with a new timeout budget.
