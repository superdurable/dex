...
When a Dex SDK starts a Flow with a positive timeout and **TimeoutPolicy** is left at **DEFAULT**, it uses **HANDLER** when the Flow has a timeout handler. Otherwise it uses **FAIL**. **HANDLER** is invalid when the Flow has no handler. A retry starts with a new timeout budget.
