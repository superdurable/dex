...
A **Flow** is the top component that you register as a Dex application.

It defines [Steps](/primitives/step) run in the background, [Attributes](/primitives/attribute) and [Channels](/primitives/channel) provide data persistence, and [RPCs](/primitives/rpc) that provide durable communication.
...
## Identifiers and Terminologies {#identifiers-and-terminologies}}

Below is the key identifiers and terminologies that are widely used in Dex.

| Identifier | Who sets it                           | Meaning                        |
|----------|---------------------------------------|-----------------------------------------------------------------|
| **Flow** | N/A                                   | Depends on the context, it usually means FlowType or FlowID        |
...
## RPC handlers {#rpc-handlers}
 ...
 Dex registers RPC methods under their method names (or explicit RPC names in Rust). Clients call them with [InvokeRPC](/primitives/rpc); then Dex server dispatch the call and the handlers run on the Worker.
