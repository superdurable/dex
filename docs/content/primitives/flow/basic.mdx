...
Each Flow explicitly declares its RPC handlers and their execution options. Go returns direct bound methods from **GetRPCs** with **DefineRPC**. Dex uses the method name as the durable RPC name, except Rust may provide an explicit name. Clients call handlers with [InvokeRPC](/primitives/rpc); Dex then dispatches the call to the Worker.
