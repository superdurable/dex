...
For token-by-token output from one long LLM call, inject a Client into the Step and write each chunk with a stable sequence of idempotency keys. The [AI agent email example](/use-cases/ai-agent-email) uses this pattern inside an async Step.
