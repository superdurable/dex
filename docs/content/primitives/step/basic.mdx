...
func (Example) WaitFor(_ dex.Context, _ int) (*dex.Wait, error) {
	return dex.Until(Approval.ForOne()), nil
}

func (Example) Execute(_ dex.Context, input int) (*dex.StepDecision, error) {
...
```java
final class Example implements Step<Integer> {
    @Override
...
class Example implements Step<number> {
...
**Until**, **AnyOf**, and **AllOf** accept Conditions without condition IDs. Do
not add IDs merely because a Condition is nested in one of these waits.
**AnyCombinationOf** is the exception: every referenced Condition needs a
unique condition ID.
...
See more in the [Parallel Steps](/design-patterns/parallel) and [Parallel SubFlows](/design-patterns/parallel-subflows) patterns.
...
See the [Failure Handling](/design-patterns/failure-handling) pattern and [money-transfer](/use-cases/money-transfer) use case.
