...
**Until**, **AnyOf**, and **AllOf** do not require condition IDs. Read Channel
results from the Channel definition and inspect Timer outcomes through the
**Context**. Add IDs only when another API requires them, such as
**AnyCombinationOf** or a Timer selected by ID.
...
When several **AnyOf** alternatives are ready in one evaluation, Dex selects
the first feasible candidate in canonical order: Timer conditions in declaration
order, then Channel conditions in declaration order, then SubFlow conditions in
declaration order. An earlier condition that is not ready does not block a later
ready condition. Mixed condition kinds therefore do not share one global call
argument order.

Use **AnyOf** for a race among the current alternatives. For strict priority
that must keep waiting for a higher-priority input, return only that condition
...
**AnyCombinationOf** is different from the flat wait combinators: every
Condition in its combinations requires a unique **condition ID**.
