...
### Runtime-selected map instances

Use the RPC definition options for fixed behavior shared by every invocation: timeout, ordinary Attribute locks, transactionality, whole-map loads, singleton Channel loads, and fixed exact-instance loads. Go names this type **RPCOptions**.

Use **RPCInvokeOptions** when the application adapter chooses an AttributeMap or ChannelMap instance from request data. Rust uses **RpcInvokeOptions**. Invocation options can add exact AttributeMap loads, exact ChannelMap loads, and AttributeMap instance locks. Dex unions, sorts, and deduplicates them with the registered definition. They cannot replace or clear registered settings.

Loading and locking are independent. A read-only lookup needs only the exact load. A read-modify-write RPC must add both the load and the lock for the same AttributeMap instance. The SDK validates the Flow ownership, definition kind, and non-empty slash-free instance before making the network call.
...
See [Hash-Partitioned AttributeMap](/design-patterns/hash-partitioned-attribute-map) for the complete lookup and upsert example, and [Sequentially Chunked AttributeMap](/design-patterns/sequentially-chunked-attribute-map) for dynamic pagination loads.
