...
An RPC can also cancel Step executions and start next Steps. See [Step cancellation](/primitives/step/step-decision#cancellation-on-stepdecision). For serializing Attribute access from RPCs, see [Attribute locking](/primitives/attribute/advanced#locking) and [RPC locking](#rpc-locking).
