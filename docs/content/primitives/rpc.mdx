...
An RPC can also cancel Step executions and start next Steps. See [Step cancellation](/primitives/step#cancellation). For serializing Attribute access from RPCs, see [Attribute locking](/primitives/attribute#locking) and [RPC locking](/references/rpc-locking).
...
them only when `api.includeRPCInputOutputIntoHistory` is enabled.

See the [RPC reference](/references/rpc).
