...
An indexed **AttributeMap** must use an explicit index key. All its instances use the same fixed key in the search schema, so a dynamic instance key cannot be the query key.
