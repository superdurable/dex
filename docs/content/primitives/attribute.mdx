---
title: Attribute
sidebar_label: Attribute
slug: /primitives/attribute
---

# Attribute

An **Attribute** lets you store data within a Flow execution, as persistence storage.

This is especially useful for intermediate data during a Flow execution. It eliminates the work of setting up and managing a database dependency.

Dex Attribute is not to replace your databases. An obvious limitation is that Attribute exists only for the lifetime of its Flow execution. Dex deletes it when it deletes the Flow execution, after the configurable retention period following Flow closure.

However, Dex allows you to sync Attribute values to your application database as permanent storage. This also lets you build more powerful database indexes to query Attributes.

## Use an Attribute {#define-an-attribute}

Define Attributes as part of the persistence schema of a Flow.

Then you can use the attribute within Step or RPC implementation to read or write the attribute. An attribute must be declared via PersistenceSchema before use. Otherwise, the worker will reject the operations.

When updating an attribute, setting it to null will delete it.

<SdkTabs
  examples={{
    python: 'examples/python/dex_examples/primitives/attribute/attribute_flow.py',
    go: 'examples/go/primitives/attribute/workflow.go',
    java: 'examples/java/src/main/java/io/superdurable/dex/primitives/attribute/AttributeFlow.java',
    typescript: 'examples/typescript/src/primitives/attribute/attribute-flow.ts',
    rust: 'examples/rust/src/primitives/attribute/flow.rs',
  }}>
<SdkSnippet lang="python">

```python
class AttributeStep(Step[str]):
    def __init__(self, flow: "AttributeFlow") -> None:
        self.flow = flow

    def wait_for(self, context: Context, input: str) -> Wait:
        self.flow.status.set(context, "processing")
        return Wait.skip_immediately()

    def execute(self, context: Context, input: str) -> StepDecision:
        self.flow.status.set(context, "completed")
        return graceful_complete(input)


class AttributeFlow(Flow[str]):
    status = Attribute(
        "primitive-attribute-status",
        str,
        index=AttributeIndex(IndexType.KEYWORD, "CustomKeyword"),
    )

    def __init__(self) -> None:
        self.start = AttributeStep(self)

    def get_steps(self) -> StepList[str]:
        return StepList.start_step(self.start)

    def get_persistence_schema(self) -> PersistenceSchema:
        return PersistenceSchema.of(self.status)
```

</SdkSnippet>
<SdkSnippet lang="go">

```go
var Status = dex.DefineAttribute[string](
	"primitive-attribute-status",
	dex.Indexed(dex.AttributeIndex{Type: dex.IndexKeyword, IndexKey: "CustomKeyword"}),
)

type AttributeFlow struct {
	dex.FlowDefaults
}

func (*AttributeFlow) GetPersistenceSchema() dex.PersistenceSchema {
	return dex.PersistenceSchema{Attributes: []dex.AttributeDef{Status}}
}

type attributeStep struct {
	dex.StepDefaults
}

func (attributeStep) WaitFor(ctx dex.Context, input string) (*dex.Wait, error) {
	if err := Status.Set(ctx, "processing"); err != nil {
		return nil, err
	}
	return dex.SkipWaitImmediately(), nil
}

func (attributeStep) Execute(ctx dex.Context, input string) (*dex.StepDecision, error) {
	if err := Status.Set(ctx, "completed"); err != nil {
		return nil, err
	}
	return dex.GracefulComplete(input), nil
}
```

</SdkSnippet>
<SdkSnippet lang="java">

```java
public final class AttributeFlow implements Flow<String> {
    private final Attribute<String> status =
            Attribute.define(
                    "primitive-attribute-status",
                    String.class,
                    new AttributeIndex(AttributeIndex.Type.KEYWORD, "CustomKeyword"));
    private final AttributeStep start = new AttributeStep();

    @Override
    public StepList<String> getSteps() {
        return StepList.startStep(start);
    }

    @Override
    public PersistenceSchema getPersistenceSchema() {
        return PersistenceSchema.of(status);
    }

    final class AttributeStep implements Step<String> {
        @Override
        public Wait waitFor(final Context context, final String input) {
            status.set(context, "processing");
            return Wait.skipImmediately();
        }

        @Override
        public StepDecision execute(final Context context, final String input) {
            status.set(context, "completed");
            return StepDecision.gracefulComplete(input);
        }
    }
}
```

</SdkSnippet>
<SdkSnippet lang="typescript">

```typescript
class AttributeStep implements Step<string> {
  public readonly inputCodec = stringCodec;

  public constructor(private readonly flow: AttributeFlow) {}

  public getStepType(): string {
    return "AttributeStep";
  }

  public waitFor(context: Context, input: string): Wait {
    this.flow.status.set(context, "processing");
    return Wait.skipImmediately();
  }

  public execute(context: Context, input: string): StepDecision {
    this.flow.status.set(context, "completed");
    return gracefulComplete(input);
  }
}

export class AttributeFlow implements Flow<string> {
  public readonly status = new Attribute("primitive-attribute-status", stringCodec, {
    type: IndexType.KEYWORD,
    indexKey: "CustomKeyword",
  });
  private readonly start = new AttributeStep(this);

  public getSteps() {
    return StepList.startStep(this.start);
  }

  public getPersistenceSchema(): PersistenceSchema {
    return { attributes: [this.status] };
  }
}
```

</SdkSnippet>
<SdkSnippet lang="rust">

```rust
pub struct AttributeFlow {
    status: Attribute<String>,
    email: Attribute<String>,
    progress: AttributeMap<String>,
    start: AttributeStep,
}

impl Default for AttributeFlow {
    fn default() -> Self {
        let status = Attribute::new("primitive-attribute-status")
            .indexed(AttributeIndex::keyword().with_key("CustomKeyword"));
        let email = Attribute::new("primitive-attribute-email").sync_to_attribute_store();
        let progress = AttributeMap::new("primitive-attribute-progress")
            .indexed(AttributeIndex::keyword().with_key("CustomKeyword2"));
        Self {
            start: AttributeStep {
                status: status.clone(),
                progress: progress.clone(),
            },
            status,
            email,
            progress,
        }
    }
}

impl Flow for AttributeFlow {
    type StartInput = String;

    fn steps(&self) -> StepList<'_, Self::StartInput> {
        StepList::start(&self.start)
    }

    fn persistence(&self) -> PersistenceSchema {
        PersistenceSchema::new()
            .attribute(&self.status)
            .attribute(&self.email)
            .attribute_map(&self.progress)
    }
}

struct AttributeStep {
    status: Attribute<String>,
    progress: AttributeMap<String>,
}

impl Step for AttributeStep {
    type Input = String;

    fn wait_for(&self, context: &mut Context, _input: Self::Input) -> HandlerResult<Wait> {
        self.status.set(context, "processing".to_owned())?;
        self.progress
            .set(context, "payment", "authorized".to_owned())?;
        Ok(Wait::skip_immediately())
    }

    fn execute(&self, context: &mut Context, input: Self::Input) -> HandlerResult<StepDecision> {
        self.status.set(context, "completed".to_owned())?;
        Ok(StepDecision::graceful_complete(input))
    }
}
```

</SdkSnippet>
</SdkTabs>

## AttributeMap {#attributemap}

An **AttributeMap** is a family of Attributes with keys decided at runtime. Use one when a Flow needs a changing set of small values, such as the status of each payment or each item in an order.

The reason for **AttributeMap** is to efficiently store a collection of data. Each Attribute is stored as a blob in Dex Server. Putting all data into a single Attribute means every update rewrites the whole blob. AttributeMap lets Dex Server store each instance as an independent blob for higher efficiency.

<SdkTabs examples={{ python: 'examples/python/dex_examples/primitives/attribute/attribute_flow.py', go: 'examples/go/primitives/attribute/workflow.go', java: 'examples/java/src/main/java/io/superdurable/dex/primitives/attribute/AttributeFlow.java', typescript: 'examples/typescript/src/primitives/attribute/attribute-flow.ts', rust: 'examples/rust/src/primitives/attribute/flow.rs' }}>
<SdkSnippet lang="python">

```python
progress = AttributeMap("primitive-attribute-progress", str)

progress.set(context, "payment", "authorized")
```

</SdkSnippet>
<SdkSnippet lang="go">

```go
var Progress = dex.DefineAttributeMap[string]("primitive-attribute-progress")

if err := Progress.Set(ctx, "payment", "authorized"); err != nil {
	return nil, err
}
```

</SdkSnippet>
<SdkSnippet lang="java">

```java
private final AttributeMap<String> progress =
        AttributeMap.define("primitive-attribute-progress", String.class);

progress.set(context, "payment", "authorized");
```

</SdkSnippet>
<SdkSnippet lang="typescript">

```typescript
public readonly progress = new AttributeMap("primitive-attribute-progress", stringCodec);

this.flow.progress.set(context, "payment", "authorized");
```

</SdkSnippet>
<SdkSnippet lang="rust">

```rust
let progress = AttributeMap::new("primitive-attribute-progress");

self.progress
    .set(context, "payment", "authorized".to_owned())?;
```

</SdkSnippet>
</SdkTabs>

## Initial Attribute values {#initial-attribute-values}

**StartFlowOptions** can set initial Attributes when the Flow is created. Initial values and Flow creation are one operation, so the first Step sees them immediately. See [Flow options](/primitives/flow/flow-options#attributes).

Applications use Flow RPCs to read and write Attributes after a Flow starts. A Client can also use **WaitForAttributeMatch** to wait until a singleton Attribute or one AttributeMap instance satisfies a scalar comparison. The call returns the matched current value. See [Client](/primitives/client/advanced#wait-for-an-attribute-match).

## Indexed Attributes {#indexed-attributes}

Dex lets you configure indexes for Attributes. With indexing, you can use the field in **SearchFlows** queries to find the Flow executions that match the query.

| Index type | Value |
| --- | --- |
| **KEYWORD** | One exact string. |
| **FULL_TEXT** | Text for tokenized search. |
| **KEYWORD_ARRAY** | A list of exact strings. |
| **INT**, **DOUBLE**, **BOOL** | The corresponding scalar value. |
| **DATETIME** | A date and time value. |

An **index key** is the key in the search schema and the key you use in a query. Index keys consume a finite, namespace-level Search Attribute pool and are shared across Flow types.

A regular Attribute uses its Attribute name as the index key by default. You can set an explicit index key instead. Plan the pool across every Flow type in the namespace. Use generic first slots such as **CustomKeyword**, **CustomText**, and **CustomInt**. Number later slots of the same type, such as **CustomKeyword2**. Two logical Attributes in the same Flow need separate physical keys. Use a business key such as **AccountID** only when its type, meaning, and query semantics remain stable throughout the namespace.

A generic key can have a different business meaning in each Flow type. Every raw visibility query that uses one must also constrain **FlowType**. Dex Web already searches within the selected Flow type. Do not index sensitive data or PII. The customer-email value in the Customer Refund example is synthetic data only.

<SdkTabs examples={{ python: 'examples/python/dex_examples/primitives/attribute/attribute_flow.py', go: 'examples/go/primitives/attribute/workflow.go', java: 'examples/java/src/main/java/io/superdurable/dex/primitives/attribute/AttributeFlow.java', typescript: 'examples/typescript/src/primitives/attribute/attribute-flow.ts', rust: 'examples/rust/src/primitives/attribute/flow.rs' }}>
<SdkSnippet lang="python">

```python
status = Attribute(
    "primitive-attribute-status",
    str,
    index=AttributeIndex(IndexType.KEYWORD, "CustomKeyword"),
)
```

</SdkSnippet>
<SdkSnippet lang="go">

```go
var Status = dex.DefineAttribute[string](
	"primitive-attribute-status",
	dex.Indexed(dex.AttributeIndex{Type: dex.IndexKeyword, IndexKey: "CustomKeyword"}),
)
```

</SdkSnippet>
<SdkSnippet lang="java">

```java
private final Attribute<String> status = Attribute.define(
        "primitive-attribute-status",
        String.class,
        new AttributeIndex(AttributeIndex.Type.KEYWORD, "CustomKeyword"));
```

</SdkSnippet>
<SdkSnippet lang="typescript">

```typescript
public readonly status = new Attribute("primitive-attribute-status", stringCodec, {
  type: IndexType.KEYWORD,
  indexKey: "CustomKeyword",
});
```

</SdkSnippet>
<SdkSnippet lang="rust">

```rust
let status = Attribute::new("primitive-attribute-status")
    .indexed(AttributeIndex::keyword().with_key("CustomKeyword"));
```

</SdkSnippet>
</SdkTabs>

For example, the status Attribute above uses **CustomKeyword** as its index key. To find completed **AttributeFlow** executions, use this query:

```text
FlowType = 'AttributeFlow' AND CustomKeyword = 'completed'
```

Pass the query to **SearchFlows**:

<SdkTabs examples={{ python: 'examples/python/dex_examples/primitives/client-apis/controller.py', go: 'examples/go/primitives/client-apis/controller.go', java: 'examples/java/src/main/java/io/superdurable/dex/primitives/clientapis/ClientApisController.java', typescript: 'examples/typescript/src/primitives/client-apis/controller.ts', rust: 'examples/rust/src/primitives/client_apis/controller.rs' }}>
<SdkSnippet lang="python">

```python
page = await app_state.client.search_flows(
    "FlowType = 'AttributeFlow' AND CustomKeyword = 'completed'",
    20,
    "",
)
```

</SdkSnippet>
<SdkSnippet lang="go">

```go
page, err := controller.client.SearchFlows(
	request.Request.Context(),
	"FlowType = 'attribute.AttributeFlow' AND CustomKeyword = 'completed'",
	20,
	"",
)
```

</SdkSnippet>
<SdkSnippet lang="java">

```java
final SearchFlowsPage page = client.searchFlows("FlowType = 'AttributeFlow' AND CustomKeyword = 'completed'", 20, "");
```

</SdkSnippet>
<SdkSnippet lang="typescript">

```typescript
const page = await client.searchFlows("FlowType = 'AttributeFlow' AND CustomKeyword = 'completed'", 20, "");
```

</SdkSnippet>
<SdkSnippet lang="rust">

```rust
client.search_flows("FlowType = 'AttributeFlow' AND CustomKeyword = 'completed'", 20)
```

</SdkSnippet>
</SdkTabs>

An indexed **AttributeMap** must use an explicit index key. It is not an index over all Map instances. Each instance write updates the same search field, so one instance can replace the value written by another instance. The Map instance key is not searchable.

This does not turn the Map into a **KEYWORD_ARRAY**. **KEYWORD_ARRAY** indexes the string array from one write; Dex does not combine the values from all Map instances. After the example below writes the payment instance with value authorized, you can query the current search value with:

```text
FlowType = 'AttributeFlow' AND CustomKeyword2 = 'authorized'
```

If another Map instance later writes a different value, that value replaces the search value for **CustomKeyword2**. Use a regular Attribute with **KEYWORD_ARRAY** when you need to search a collection of values. Do not use an AttributeMap index to find every instance with a value.

<SdkTabs examples={{ python: 'examples/python/dex_examples/primitives/attribute/attribute_flow.py', go: 'examples/go/primitives/attribute/workflow.go', java: 'examples/java/src/main/java/io/superdurable/dex/primitives/attribute/AttributeFlow.java', typescript: 'examples/typescript/src/primitives/attribute/attribute-flow.ts', rust: 'examples/rust/src/primitives/attribute/flow.rs' }}>
<SdkSnippet lang="python">

```python
progress = AttributeMap(
    "primitive-attribute-progress",
    str,
    index=AttributeIndex(IndexType.KEYWORD, "CustomKeyword2"),
)
```

</SdkSnippet>
<SdkSnippet lang="go">

```go
var Progress = dex.DefineAttributeMap[string](
	"primitive-attribute-progress",
	dex.Indexed(dex.AttributeIndex{Type: dex.IndexKeyword, IndexKey: "CustomKeyword2"}),
)
```

</SdkSnippet>
<SdkSnippet lang="java">

```java
private final AttributeMap<String> progress =
        AttributeMap.define(
                "primitive-attribute-progress",
                String.class,
                new AttributeIndex(AttributeIndex.Type.KEYWORD, "CustomKeyword2"));
```

</SdkSnippet>
<SdkSnippet lang="typescript">

```typescript
public readonly progress = new AttributeMap("primitive-attribute-progress", stringCodec, {
  type: IndexType.KEYWORD,
  indexKey: "CustomKeyword2",
});
```

</SdkSnippet>
<SdkSnippet lang="rust">

```rust
let progress = AttributeMap::new("primitive-attribute-progress")
    .indexed(AttributeIndex::keyword().with_key("CustomKeyword2"));
```

</SdkSnippet>
</SdkTabs>

## Attribute Sync {#attribute-store-projection}

Dex Attribute is not to replace your databases. Attribute Sync feature will sync the attribute values to your application database, without you writing a single line of code.

So that you can store the attributes in your database as permanent storage. This also lets you build more powerful database indexes to query Attributes.

Mark each Attribute that should sync:

<SdkTabs examples={{ python: 'examples/python/dex_examples/patterns/entity-store/user_profile_flow.py', go: 'examples/go/patterns/entity-store/workflow.go', java: 'examples/java/src/main/java/io/superdurable/dex/patterns/entitystore/UserProfileFlow.java', typescript: 'examples/typescript/src/patterns/entity-store/user-profile-flow.ts', rust: 'examples/rust/src/patterns/entity_store/flow.rs' }}>
<SdkSnippet lang="python">

```python
display_name = Attribute("display_name", str, sync_to_attribute_store=True)
```

</SdkSnippet>
<SdkSnippet lang="go">

```go
var DisplayName = dex.DefineAttribute[string](
	"display_name",
	dex.SyncToAttributeStore(),
)
```

</SdkSnippet>
<SdkSnippet lang="java">

```java
public final Attribute<String> displayName =
        Attribute.define("display_name", String.class).syncToAttributeStore();
```

</SdkSnippet>
<SdkSnippet lang="typescript">

```typescript
public readonly displayName = new Attribute("display_name", stringCodec)
  .syncToAttributeStore();
```

</SdkSnippet>
<SdkSnippet lang="rust">

```rust
static DISPLAY_NAME: LazyLock<Attribute<String>> =
    LazyLock::new(|| Attribute::new("display_name").sync_to_attribute_store());
```

</SdkSnippet>
</SdkTabs>

First, define an Attribute Store in Dex Server configuration:

```yaml
attributeStore:
  stores:
    entityStore:
      type: postgres
      dsn: postgres://entity_store:entity_store@localhost:55432/entity_store?sslmode=disable
      tableName: public.user_profiles
```

The same configuration can target MySQL, Databricks, Snowflake, or MongoDB.
Create the destination first. Attribute Sync writes latest state asynchronously
and does not automatically backfill existing Flows. See
[Custom Attribute Store operations](/production/attribute-store) for target
contracts, type mappings, and retry behavior.

Start Dex Server with the configuration:

```bash
dexcli dev --attribute-store-config ./attribute-store.yaml
```

Then choose the same store name when starting the Flow:

<SdkTabs examples={{ python: 'examples/python/dex_examples/patterns/entity-store/controller.py', go: 'examples/go/patterns/entity-store/controller.go', java: 'examples/java/src/main/java/io/superdurable/dex/patterns/entitystore/EntityStoreController.java', typescript: 'examples/typescript/src/patterns/entity-store/controller.ts', rust: 'examples/rust/src/patterns/entity_store/flow.rs' }}>
<SdkSnippet lang="python">

```python
options = StartFlowOptions(
    config_override=FlowConfig(attribute_store_names=["entityStore"]),
)
await client.start_flow(flow, user_id, None, options)
```

</SdkSnippet>
<SdkSnippet lang="go">

```go
options := dex.StartFlowOptions{
	ConfigOverride: &dex.FlowConfig{
		AttributeStoreNames: []string{"entityStore"},
	},
}
_, err := client.StartFlow(ctx, flow, userID, nil, options)
```

</SdkSnippet>
<SdkSnippet lang="java">

```java
final StartFlowOptions options = StartFlowOptions.newBuilder()
        .configOverride(FlowConfig.newBuilder()
                .attributeStoreNames("entityStore")
                .build())
        .build();
final String runId = client.startFlow(flow, userId, null, options);
```

</SdkSnippet>
<SdkSnippet lang="typescript">

```typescript
const runId = await client.startFlow(flow, userId, undefined, {
  configOverride: { attributeStoreNames: ["entityStore"] },
});
```

</SdkSnippet>
<SdkSnippet lang="rust">

```rust
let options = StartFlowOptions::new()
    .config_override(FlowConfig::new().attribute_store_names(vec!["entityStore".to_owned()]));
let run_id = client.start_flow_with_options(&flow, &user_id, (), options)?;
```

</SdkSnippet>
</SdkTabs>

## Locking {#locking}

Concurrent Steps and RPC invocations can read and write the same Attribute. This can lead to race conditions. To avoid this, use Attribute locks. Dex Server ensures only one Step execution or RPC handling can acquire the lock at a time.

### Lock **WaitFor**

<SdkTabs
  examples={{
    python: 'examples/python/dex_examples/primitives/attribute/attribute_flow.py',
    go: 'examples/go/primitives/attribute/workflow.go',
    java: 'examples/java/src/main/java/io/superdurable/dex/primitives/attribute/AttributeFlow.java',
    typescript: 'examples/typescript/src/primitives/attribute/attribute-flow.ts',
    rust: 'examples/rust/src/primitives/attribute/flow.rs',
  }}>
<SdkSnippet lang="python">

```python
def get_step_options(self) -> StepOptions:
    return StepOptions(
        wait_for_lock_attributes=(
            self.flow.status.lock(),
            self.flow.progress.lock("payment"),
        ),
    )
```

</SdkSnippet>
<SdkSnippet lang="go">

```go
func (attributeStep) GetStepOptions() *dex.StepOptions {
	return &dex.StepOptions{
		WaitForLockAttributes: []dex.AttributeLock{
			dex.LockAttribute(Status),
			dex.LockAttributeMap(Progress, "payment"),
		},
	}
}
```

</SdkSnippet>
<SdkSnippet lang="java">

```java
return StepOptions.newBuilder()
        .addWaitForLock(AttributeLock.of(status))
        .addWaitForLock(AttributeLock.of(progress, "payment"))
        .build();
```

</SdkSnippet>
<SdkSnippet lang="typescript">

```typescript
return {
  waitForLockAttributes: [this.flow.status.lock(), this.flow.progress.lock("payment")],
};
```

</SdkSnippet>
<SdkSnippet lang="rust">

```rust
StepOptions::new()
    .wait_for_lock(self.status.lock())
    .wait_for_lock(self.progress.lock("payment"))
```

</SdkSnippet>
</SdkTabs>

### Lock **Execute**

<SdkTabs
  examples={{
    python: 'examples/python/dex_examples/primitives/attribute/attribute_flow.py',
    go: 'examples/go/primitives/attribute/workflow.go',
    java: 'examples/java/src/main/java/io/superdurable/dex/primitives/attribute/AttributeFlow.java',
    typescript: 'examples/typescript/src/primitives/attribute/attribute-flow.ts',
    rust: 'examples/rust/src/primitives/attribute/flow.rs',
  }}>
<SdkSnippet lang="python">

```python
def get_step_options(self) -> StepOptions:
    return StepOptions(
        execute_lock_attributes=(
            self.flow.status.lock(),
            self.flow.progress.lock("payment"),
        ),
    )
```

</SdkSnippet>
<SdkSnippet lang="go">

```go
func (attributeStep) GetStepOptions() *dex.StepOptions {
	return &dex.StepOptions{
		ExecuteLockAttributes: []dex.AttributeLock{
			dex.LockAttribute(Status),
			dex.LockAttributeMap(Progress, "payment"),
		},
	}
}
```

</SdkSnippet>
<SdkSnippet lang="java">

```java
return StepOptions.newBuilder()
        .addExecuteLock(AttributeLock.of(status))
        .addExecuteLock(AttributeLock.of(progress, "payment"))
        .build();
```

</SdkSnippet>
<SdkSnippet lang="typescript">

```typescript
return {
  executeLockAttributes: [this.flow.status.lock(), this.flow.progress.lock("payment")],
};
```

</SdkSnippet>
<SdkSnippet lang="rust">

```rust
StepOptions::new()
    .execute_lock(self.status.lock())
    .execute_lock(self.progress.lock("payment"))
```

</SdkSnippet>
</SdkTabs>

### Lock an RPC

<SdkTabs
  examples={{
    python: 'examples/python/dex_examples/primitives/attribute/attribute_flow.py',
    go: 'examples/go/primitives/attribute/workflow.go',
    java: 'examples/java/src/main/java/io/superdurable/dex/primitives/attribute/AttributeFlow.java',
    typescript: 'examples/typescript/src/primitives/attribute/attribute-flow.ts',
    rust: 'examples/rust/src/primitives/attribute/flow.rs',
  }}>
<SdkSnippet lang="python">

```python
@rpc(lock_attributes=(status.lock(), progress.lock("payment")))
def update_status(self, context: Context, input: str) -> RPCResult[str]:
    self.status.set(context, input)
    self.progress.set(context, "payment", input)
    return RPCResult(input)
```

</SdkSnippet>
<SdkSnippet lang="go">

```go
func (flow *AttributeFlow) GetRPCs() []dex.RPCDef {
	return []dex.RPCDef{
		dex.DefineRPC(flow.UpdateStatus, &dex.RPCOptions{
			LockAttributes: []dex.AttributeLock{
				dex.LockAttribute(Status),
				dex.LockAttributeMap(Progress, "payment"),
			},
		}),
	}
}
```

</SdkSnippet>
<SdkSnippet lang="java">

```java
@RPC(
        lockAttributes = {"primitive-attribute-status"},
        lockAttributeMaps = {
            @RPCAttributeMapLock(
                    attribute = "primitive-attribute-progress",
                    instance = "payment")
        })
public RPCResult<String> updateStatus(final Context context, final String input) {
    status.set(context, input);
    progress.set(context, "payment", input);
    return RPCResult.of(input);
}
```

</SdkSnippet>
<SdkSnippet lang="typescript">

```typescript
@rpc({
  inputCodec: stringCodec,
  outputCodec: stringCodec,
  lockAttributes: [status.lock(), progress.lock("payment")],
})
public updateStatus(context: Context, input: string): RPCResult<string> {
  this.status.set(context, input);
  this.progress.set(context, "payment", input);
  return { output: input };
}
```

</SdkSnippet>
<SdkSnippet lang="rust">

```rust
fn rpcs(&self) -> RpcList<Self> {
    RpcList::new().function(
        UPDATE_STATUS
            .lock(self.status.lock())
            .lock(self.progress.lock("payment")),
        Self::update_status,
    )
}
```

</SdkSnippet>
</SdkTabs>
