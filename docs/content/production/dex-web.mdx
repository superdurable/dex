...
Each Action requires one permission such as **refund.manage**. Permission keys begin with a lowercase letter and contain lowercase letters, digits, dots, or hyphens. The Go Worker evaluates every Action condition after relevant state changes and projects the deduplicated permissions of currently available Actions into the internal **DexWorkQueuePermissions** KeywordList Search Attribute.
