...
On **v2**, selecting a Step on the canvas opens a detail panel for the current run. **Input**, **Output**, and **Context** show the selected WaitFor or Execute history event in the same structured form as **v1** Selected event. The panel defaults to Details and can switch to Raw JSON of the hydrated payload. AttributeMap and ChannelMap instances that are decimal numbers sort by numeric value, matching **v1**. Execute shows the live next Steps from that run's decision, not every FDG exit. When Continue-as-New moved a Step into a previous run, use Load more from previous run.
