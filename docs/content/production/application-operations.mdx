...
For handler policy, the internal `sys:timeout_handler` timer is visible in
active-Step diagnostics and may be fired early with `SkipTimer`. Mitigate a
failed timeout with reset only after fixing the underlying cause.
