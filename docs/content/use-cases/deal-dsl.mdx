...
Open the image for the original 3200 × 3600 PNG. It was captured directly at 2× device scale; it was not enlarged after capture.

Run the full local Go demo from **examples/go**:

```bash
KEEP_DEAL_DSL_DEMO=1 make dealDSLDemo
```

The demo starts PostgreSQL, Dex, the Deal DSL Worker, and the Deal UI. It executes completed purchase and refund paths plus a Flow that remains waiting for buyer input. Python, Go, Java, TypeScript, and Rust also include automated end-to-end tests that start the Flow, send the buyer-decision Channel message, and verify the final item-delivery state.
