...
Regular user messages enter the durable **UserMessages** FIFO queue. While the Agent loop is running, they remain pending instead of interrupting the current model or tool call. The UI shows them immediately and lets the user edit, delete, or Steer a message while it is still pending. Editing removes the pending message first; resubmitting appends a new message with a new ID at the tail.

Steer transactionally deletes one selected regular message and republishes its value to **ImmediateUserMessages**. The Agent checks that queue before its next model call, tool, approval wait, or Timer continuation. It does not cancel an LLM or MCP request already running. At the next safe boundary, the Agent clears unexecuted tool calls and stale approval or Timer state, records structured cancellation results, and replans from the Steer message.
...
The built-in **durable_wait** tool moves the Flow to a Step that waits on both a [Timer](/primitives/timer) and **ImmediateUserMessages**. The Timer can survive Worker restarts. A regular message remains queued; choosing Steer interrupts the wait and lets the Agent replan.

## Live events

Reasoning summaries and assistant text use separate buffered [Streams](/primitives/stream). The Step passes both buffered writers to the model adapter. The SDK combines small chunks for up to one second or 16 KiB and flushes each tail before the Step result or error.
...
Messages submitted during active work appear between the conversation and composer as **Queued**. Use **Edit** or **Delete** before consumption, or **Steer** to apply one at the next safe boundary.
