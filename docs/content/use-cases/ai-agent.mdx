...
The built-in **durable_wait** tool moves the Flow to a Step that waits on both a [Timer](/primitives/timer) and **SteeredUserMessages**. The Timer can survive Worker restarts. A queued message remains queued; choosing Steer interrupts the wait and lets the Agent replan.
