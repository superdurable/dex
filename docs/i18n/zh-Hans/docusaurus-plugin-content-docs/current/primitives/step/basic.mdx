...
**Until**、**AnyOf** 和 **AllOf** 接受没有 condition ID 的 Condition。不要仅仅因为
Condition 嵌套在这些等待组合中就为它添加 ID。**AnyCombinationOf** 是例外：其中引用的
每个 Condition 都需要唯一的 condition ID。
...
见 [并行 Step](/design-patterns/parallel) 与 [并行 SubFlow](/design-patterns/parallel-subflows) 模式。
...
见 [Failure Handling](/design-patterns/failure-handling) 模式与 [money-transfer](/use-cases/money-transfer) 用例。
