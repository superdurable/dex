...
每种 Step 类型最多实现 两个短方法。WaitFor 注册 durable Condition 并返回 Wait。Condition 完成后，Dex 重新 hydrate 该 Step 并调用 Execute。Execute 返回 **StepDecision** 推动 Flow 前进。
...
关闭决策、取消、Flow 输出与 forceCompleteIfChannelsEmpty 见 [Step decision](/primitives/step/step-decision)。
