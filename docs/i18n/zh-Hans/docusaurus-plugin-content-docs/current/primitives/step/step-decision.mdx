---
title: 高级 Step decision
sidebar_label: 高级 Step decision
...
本分支关闭 Flow 时，GracefulComplete(output) 设置 Flow 输出。Step 等待 [SubFlow](/primitives/subflow) 时，在 Execute 中通过 SubFlowResult / getConditionResults / condition_result 读取子输出（见 [高级等待条件 — SubFlow 结果](/primitives/step/waiting-condition#condition-results)）。
