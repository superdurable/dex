...
RPC 可以把删除与其他 durable write 组合。Move 示例加载 **Queued**，按 message ID 找到原始 Value，再暂存 deletion 并重新发布该 Value。Caller 只传 ID。Missing message 必须中止所有 write 时，需要启用 transactional execution；见 [Transactional read 与 write](/primitives/rpc#transactional-reads-and-writes)。
