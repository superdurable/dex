...
Flow 启动后，应用通过 Flow RPC 读写 Attribute。Client 也可以用 **WaitForAttributeMatch**，等待 singleton Attribute 或某个 AttributeMap instance 满足 scalar 比较，并返回当前命中的值。见 [Client](/primitives/client/advanced#wait-for-an-attribute-match)。
...
**index key** 是 search schema 中的 key，也是 query 中使用的 key。index key 会消耗 namespace 级、数量有限的 Search Attribute 槽位，并且跨 Flow type 共用。

普通 Attribute 默认用 Attribute name 作为 index key。你也可以设置显式 index key。请在 namespace 的所有 Flow type 之间规划这些槽位。首个通用槽位使用 **CustomKeyword**、**CustomText**、**CustomInt** 等名称；同一类型的后续槽位使用 **CustomKeyword2** 这样的编号名称。同一个 Flow 中的两个逻辑 Attribute 需要不同的物理 key。只有当 **AccountID** 这类业务 key 的类型、含义和 query 语义在整个 namespace 内都稳定时，才使用它。

通用 key 在不同 Flow type 中可以有不同的业务含义。任何使用通用 key 的 raw visibility query 都必须同时限定 **FlowType**。Dex Web 已经在选定的 Flow type 内搜索。不要索引敏感数据或 PII。Customer Refund 示例中的 customer-email 值仅为合成数据。
...
上面的 status Attribute 使用 **CustomKeyword** 作为 index key。要找 status 为 completed 的 **AttributeFlow** execution，可以使用这个 query：
...
如果之后另一个 Map instance 写入不同的值，它会替换 **CustomKeyword2** 的 search value。需要搜索一组值时，应使用带 **KEYWORD_ARRAY** 的普通 Attribute。不要用 AttributeMap index 查找所有有某个值的 instance。
...
同一配置也可以使用 MySQL、Databricks、Snowflake 或 MongoDB。请先创建目标。
Attribute Sync 会异步写入最新状态，不会自动回填已有 Flow。目标契约、类型映射和
重试行为请参阅[自定义 Attribute Store 运维](/production/attribute-store)。
