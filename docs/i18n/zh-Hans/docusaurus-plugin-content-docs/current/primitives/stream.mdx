...
如果一次很长的 LLM call 需要逐 token 输出，可以向 Step 注入 Client，并为每个 chunk 使用稳定、有序的 idempotency key。[AI agent email example](/use-cases/ai-agent-email) 在 async Step 中使用了这种方式。
