...
下面的 Step 使用 buffered text writer。它显式 flush 第一条 progress message，Dex 在 Step result 之前 flush 第二条。Handler 不会等待 Stream Store acknowledgement。
...
当许多小 chunk 共同组成一段文本时，例如 LLM delta，请使用 buffered text writer。默认 flush interval 是一秒，soft threshold 是 16 KiB UTF-8 数据。Timer、size threshold、手动 flush 或 invocation 完成时，都会发送当前非空 batch。Helper 会原样保留文本，不拆分单个 chunk，并忽略空 chunk。
