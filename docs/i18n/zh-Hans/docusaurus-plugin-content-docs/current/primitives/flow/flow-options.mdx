...
Flow 有 timeout handler 时，Dex 默认使用 **HANDLER**；否则使用 **FAIL**。没有 handler 时使用 **HANDLER** 无效。continue-as-new 会保留当前 deadline。一次 retry 会获得新的 timeout budget。
...
调用方可能在丢失 StartFlow response 后重试时，应使用稳定的 **RequestID**。重试时发送相同的 Flow ID、start input、options 与 RequestID。启用 **AlreadyStartedOptions.IgnoreError** 后，Dex 只会在已有 Flow 使用相同 RequestID 启动时返回其 Run ID。不同 RequestID 仍会得到 already-started error。这个选项不会 attach 到无关的 Flow。
...
[SubFlow](/primitives/subflow) condition 使用 **SubFlowOptions** 设置 timeout、retry、初始 Attribute 与 **FlowConfig** override。SubFlow 还有生成 Flow ID 的 reuse policy。Dex 负责这个 start ID，调用方无法覆盖。

## Operational APIs {#operational-apis}

[Client APIs](/primitives/client-apis) 还可以 search Flow、调用 RPC、publish Channel、wait、stop、skip timer、time travel 与 continue as new。
