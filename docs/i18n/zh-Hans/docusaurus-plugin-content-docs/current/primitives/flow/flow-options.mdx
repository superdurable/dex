...
**CronSchedule** 按计划启动新的 execution。retry 用来恢复一次失败的 execution，而 cron 创建之后的 execution。overlap 与漏掉运行怎么处理由 backend 决定。

### StartDelay {#start-delay}

**StartDelay** 会立即接受要启动的 Flow，但在 **StartStep** 可以运行前等待。它适合一次性延迟，例如新订单创建五分钟后再处理。周期性 schedule 请使用 **CronSchedule**。
