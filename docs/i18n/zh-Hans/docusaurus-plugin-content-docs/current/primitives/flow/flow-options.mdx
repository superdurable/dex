...
当 Dex SDK 以正数 timeout 启动 Flow，并且 **TimeoutPolicy** 保持 **DEFAULT** 时，有 timeout handler 就使用 **HANDLER**；否则使用 **FAIL**。没有 handler 时使用 **HANDLER** 无效。一次 retry 会获得新的 timeout budget。
