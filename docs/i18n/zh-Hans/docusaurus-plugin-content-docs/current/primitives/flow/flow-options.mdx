...
如需在 Flow 截止时间执行应用定义的处理，请参阅 [Graceful Timeout](/design-patterns/failure-handling/graceful-timeout)。
...
|-------------------|------------------|------------------|
| **IDReuseDefault** | 拒绝启动。RequestID 相同且启用 **IgnoreError** 时返回 active Run ID。 | 拒绝启动。RequestID 相同且启用 **IgnoreError** 时返回已有 Run ID。 |
...
Dex Server 要求 **RequestID** 非空。省略时，SDK 会在发送请求前生成它。服务端将它记录在 Flow execution 中，再使用请求的 ID reuse policy 让已配置的 backend 启动 Flow。**IDReuseDefault** 使用服务端默认行为：拒绝所有曾经使用过的 Flow ID。需要让已关闭的 Flow ID 启动新 execution 时，必须显式选择 **IDReuseAllowIfNotRunning**。
...
| 字段 | 行为 |
|------|------|
| **ActiveStepSearchMode** | 索引所有 active Step、只索引运行过 **WaitFor** 的 Step，或不索引 active Step。内置 Server 默认不索引 active Step；如需启用，请显式设置模式。 |
