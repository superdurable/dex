...
每个 Flow 都显式声明 RPC handler 及其 execution options。Go 通过 **GetRPCs** 和 **DefineRPC** 返回 direct bound method。Dex 将 method name 作为 durable RPC name，Rust 也可以提供显式名称。Client 使用 [InvokeRPC](/primitives/rpc) 调用；Dex 随后将调用派发到 Worker。
