---
title: RPC
sidebar_label: RPC
slug: /primitives/rpc
---

import RpcRequestDiagram from '@site/src/components/primitives/rpc/RpcRequestDiagram';

# RPC

RPC 是 “Remote Procedure Call” 的缩写，允许外部系统与 Flow execution 交互。

**Client** 发起 RPC。**Dex Server** 将它发送给 **Worker**。**Worker** 执行后，将结果返回给 **Dex Server** 持久化。**Dex Server** 再将输出返回给 **Client**。

<RpcRequestDiagram />

## 定义和调用 RPC

RPC 可以通过 Attribute 读取和写入 durable state、向 Channel publish，也可以 trigger 一个全新的 Step 来运行。

每次 RPC 调用都是一个 commit 边界。Dex Server 会暂存 RPC 中的 Attribute 写入和 Channel publish，并只在 method 成功返回时，将它们与 RPC result 一起 commit。如果 RPC 失败，这些 durable 变更都不会被 commit。外部 API 调用不在这个原子边界内。

<SdkTabs
  examples={{
    python: 'examples/python/dex_examples/primitives/rpc',
    go: 'examples/go/primitives/rpc',
    java: 'examples/java/src/main/java/io/superdurable/dex/primitives/rpc',
    typescript: 'examples/typescript/src/primitives/rpc',
    rust: 'examples/rust/src/primitives/rpc',
  }}>
<SdkSnippet lang="python">

```python
@rpc
def trigger(self, context: Context, input: str) -> RPCResult[str]:
    self.data.set(context, input)
    self.example_ch.publish(context, None)
    return RPCResult(input, next_steps=(StepMovement.of(ExampleStep, input),))
```

</SdkSnippet>
  <SdkSnippet lang="go">

```go
func (*RpcFlow) Trigger(ctx dex.Context, input string) (*dex.RPCResult[string], error) {
	if err := Data.Set(ctx, input); err != nil {
		return nil, err
	}
	if err := ExampleCh.Publish(ctx, nil); err != nil {
		return nil, err
	}
	return &dex.RPCResult[string]{
		Output: input,
		NextSteps: []dex.StepMovement{
			dex.MovementOf(exampleStep{}, input),
		},
	}, nil
}
```

</SdkSnippet>
  <SdkSnippet lang="java">

```java
@RPC
public RPCResult<String> trigger(final Context context, final String input) {
    data.set(context, input);
    exampleCh.publish(context, null);
    return RPCResult.of(input, StepMovement.of(ExampleStep.class, input));
}
```

</SdkSnippet>
  <SdkSnippet lang="typescript">

```typescript
@rpc({ inputCodec: stringCodec, outputCodec: stringCodec })
public trigger(context: Context, input: string): RPCResult<string> {
  this.data.set(context, input);
  exampleCh.publish(context, undefined);
  return { output: input, nextSteps: [StepMovement.of(ExampleStep, input)] };
}
```

</SdkSnippet>
  <SdkSnippet lang="rust">

```rust
impl RpcFlow {
    fn trigger(&self, context: &mut Context, input: String) -> HandlerResult<RpcResult<String>> {
        data().set(context, input.clone())?;
        example_ch().publish(context, ())?;
        Ok(RpcResult::new(input.clone()).then(StepMovement::to(&self.example_step, input)))
    }
}
```

</SdkSnippet>
</SdkTabs>

### Java 和 Kotlin

**Client.newRpcStub** 会创建 Flow 的子类。Java Flow class 和它的 RPC method 都不能是 final。Kotlin 默认将两者都设为 final，所以要将 Flow class 和它的 RPC method 声明为 **open**。

### TypeScript codec

TypeScript 未设置 **inputCodec** 或 **outputCodec** 时会使用 JSON。input 或 output 不是 JSON 时需要提供 codec，包括 string、number、boolean 和 bigint 等 scalar value。

然后像下面的例子一样调用 RPC：

<SdkTabs
  examples={{
    python: 'examples/python/dex_examples/primitives/rpc',
    go: 'examples/go/primitives/rpc',
    java: 'examples/java/src/main/java/io/superdurable/dex/primitives/rpc',
    typescript: 'examples/typescript/src/primitives/rpc',
    rust: 'examples/rust/src/primitives/rpc',
  }}>
<SdkSnippet lang="python">

```python
result = await client.invoke_rpc(flow.trigger, flow_id, message)
```

</SdkSnippet>
  <SdkSnippet lang="go">

```go
var result string
err := client.InvokeRPC(
	ctx,
	flowID,
	flow.Trigger,
	message,
	&result,
	sdk.InvokeOptions{},
)
```

</SdkSnippet>
  <SdkSnippet lang="java">

```java
final RpcFlow flow = client.newRpcStub(RpcFlow.class, flowId);
final String result = client.invokeRPC(flow::trigger, message);
```

</SdkSnippet>
  <SdkSnippet lang="typescript">

```typescript
const result = await client.invokeRPC(flow.trigger, flowId, message);
```

</SdkSnippet>
  <SdkSnippet lang="rust">

```rust
let result = client.invoke_rpc(&workflow_id, RPC_TRIGGER, message)?;
```

</SdkSnippet>
</SdkTabs>

## Transactional read 与 write {#transactional-reads-and-writes}

持有 Attribute lock 的 RPC 已经会使用 Temporal synchronous Update execution。它的 read、Channel deletion check 与 write 会使用同一个 transactional Flow-state snapshot。

只有 Channel deletion 时不会自动选择这条路径。RPC 必须检查 pending message ID，并把删除与 Attribute write、Channel publication、event 或 Step decision 一起 commit 时，需要显式标记为 transactional。任何目标 message 不存在时，Dex 会拒绝 RPC，而且不会 commit 其中任何 write。

Controller 先读取 pending message，再把 ID 和 Value 传给 transactional RPC。如果 concurrent consumer 已取走该消息，删除会失败，因此 Dex 不会发布 stale copy：

<SdkTabs
  examples={{
    python: 'examples/python/dex_examples/primitives/channel/channel_flow.py',
    go: 'examples/go/primitives/channel/controller.go',
    java: 'examples/java/src/main/java/io/superdurable/dex/primitives/channel/ChannelFlow.java',
    typescript: 'examples/typescript/src/primitives/channel/channel-flow.ts',
    rust: 'examples/rust/src/primitives/channel/flow.rs',
  }}>
<SdkSnippet lang="python">

```python
@rpc(is_transactional=True)
def move(self, context: Context, input: MoveMessage) -> None:
    self.queued.delete(context, input.message_id)
    self.moved.publish(context, input.value)
```

</SdkSnippet>
<SdkSnippet lang="go">

```go
err = controller.client.InvokeRPC(
	request.Request.Context(),
	flowID,
	controller.flow.Move,
	MoveMessage{MessageID: messageToMove.MessageID, Value: messageToMove.Value},
	&none,
	sdk.InvokeOptions{IsTransactional: true},
)
```

</SdkSnippet>
<SdkSnippet lang="java">

```java
@RPC(isTransactional = true)
public void move(final Context context, final MoveMessage message) {
    queued.delete(context, message.messageId);
    moved.publish(context, message.value);
}
```

</SdkSnippet>
<SdkSnippet lang="typescript">

```typescript
@rpc({ isTransactional: true, inputCodec: moveMessageCodec })
public move(context: Context, message: MoveMessage): void {
  queued.delete(context, message.messageId);
  moved.publish(context, message.value);
}
```

</SdkSnippet>
<SdkSnippet lang="rust">

```rust
fn rpcs(&self) -> RpcList<Self> {
    RpcList::new()
        .procedure_without_input(CHANNEL_APPROVE, Self::approve)
        .procedure(CHANNEL_MOVE.is_transactional(), Self::move_message)
}
```

</SdkSnippet>
</SdkTabs>

Non-transactional signal RPC 遇到 missing deletion 时会把它当作 no-op，仍然 commit 其他成功的 effect。Cadence 不提供 Temporal synchronous Update；Dex 会降级为 query 加 signal，因此 deletion check 与 write 不具有相同的 atomic guarantee。Cadence caller 必须能容忍这项竞态。

## RPC timeout

RPC timeout 限制一次 Worker handler invocation。

Python、Java、TypeScript 和 Rust 在 RPC handler 上声明 timeout。Go 在每次 Client 调用的 **InvokeOptions** 中传入。应用不设置 timeout 时，Dex Server 使用自己的 default。默认最大值为 60 秒。

<SdkTabs
  examples={{
    python: 'examples/python/dex_examples/primitives/rpc',
    go: 'examples/go/primitives/rpc',
    java: 'examples/java/src/main/java/io/superdurable/dex/primitives/rpc',
    typescript: 'examples/typescript/src/primitives/rpc',
    rust: 'examples/rust/src/primitives/rpc',
  }}>
<SdkSnippet lang="python">

```python
@rpc(timeout=timedelta(seconds=30))
```

</SdkSnippet>
<SdkSnippet lang="go">

```go
sdk.InvokeOptions{Timeout: 30 * time.Second}
```

</SdkSnippet>
<SdkSnippet lang="java">

```java
@RPC(timeoutSeconds = 30)
```

</SdkSnippet>
<SdkSnippet lang="typescript">

```typescript
@rpc({ inputCodec: stringCodec, outputCodec: stringCodec, timeoutMs: 30_000 })
```

</SdkSnippet>
<SdkSnippet lang="rust">

```rust
RpcList::new().function(RPC_TRIGGER.timeout(Duration::from_secs(30)), Self::trigger)
```

</SdkSnippet>
</SdkTabs>

## Attribute locks

Attribute lock 请参阅 [Locking](/primitives/attribute#locking)。
