...
**ManualRecoveryFlow** 从 **DoWork** 开始。它的失败策略执行三次指数退避重试，然后转到 **ManualStep**。**ManualStep** 等待 **Retry** Channel 或 **Skip** Channel。操作人员解决问题后，Retry 回到 **DoWork**；Skip 使用明确原因使 Flow 失败。
