...
**DoWorkStep** 接收一个布尔输入。true 会立即失败；false 会完成 Flow。它的失败策略使用指数退避重试三次，然后转入 **ManualStep**。四次总尝试包含最初的一次执行。

**ManualStep** 等待 **Retry Channel** 或 **Skip Channel**。问题解决后，人工发布 **Retry Channel**，示例会用 false 输入再次运行 **DoWorkStep**。发布 **Skip Channel** 会以明确原因使 Flow 失败。

## 核心实现

每个示例都定义失败策略和人工决策。完整示例还注册了 Flow 并提供 HTTP 启动端点。
...
- [Step 选项](/primitives/step/step-options)
- [失败恢复](/design-patterns/recovery)
