...
**SubFlows.WaitFor** 为每个请求创建一个 SubFlow 条件，并用 **allOf** 组合。只有所有子 Flow 都结束后，父 Flow 才会继续。当不需要部分完成或取消时，这是最简单的父子 fan-out。
...
func (subFlowsStep) GetStepType() string { return "SubFlows" }
...
```typescript
class SubFlows implements Step<readonly string[]> {
...
export class BasicParentFlow implements Flow<readonly string[]> {
  ...
  public constructor(exampleSubFlow: ExampleSubFlow) {
    this.subFlows = new SubFlows(exampleSubFlow);
  }
