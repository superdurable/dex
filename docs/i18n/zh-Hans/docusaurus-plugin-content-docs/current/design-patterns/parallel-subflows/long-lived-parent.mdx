...
**Init** 将初始请求发布到 **RequestChannel**，把 **Stopped** 设为 false，并启动 N 个 **HandleRequest**。每个 worker 取得一条 FIFO 消息后跳转到 **HandleSubFlow**。后者启动并等待 **ExampleSubFlow**；完成后检查 **Stopped**，决定回到 **HandleRequest** 还是 graceful complete。
