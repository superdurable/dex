...
import flowDefinition from '@site/src/data/flow-definitions/design-patterns/parallel-subflows-wait-for-half.json';
...
每个子 Flow 都在独立的 **SubFlowStep** execution 中运行。子 Flow 完成后向 **SubFlowCompletedCh** 发布一条消息。**WaitSubFlows** 等待其中一半消息，然后为每个可能尚未完成的子 Flow 发布一条 **AllDoneCh** 消息。
...
class WaitForHalfParentFlow(Flow[list[str]]):
    ...
    def __init__(
        ...
        self.wait_subflows = WaitSubFlows(self.subflow_completed_ch, self.all_done_ch)
...
func (waitSubFlowsStep) GetStepType() string { return "WaitSubFlows" }
...
class WaitForHalfInit implements Step<readonly string[]> {
  ...
  public getStepType(): string { return "Init"; }
  public execute(_context: Context, requests: readonly string[]): StepDecision {
    if (requests.length === 0) return gracefulComplete();
    return goToMany(StepMovement.of(WaitSubFlows, requests.length), ...requests.map((request) => StepMovement.of(SubFlowStep, request)));
  }
}
...
class WaitSubFlows implements Step<number> {
  ...
  public getStepType(): string { return "WaitSubFlows"; }
...
impl Step for WaitForHalfInit {
    ...
    fn execute(&self, _context: &mut Context, requests: Self::Input) -> HandlerResult<StepDecision> {
        ...
        let mut movements = vec![StepMovement::to(&WaitSubFlows, requests.len())];
