...
<img
  alt="排空外部 Channel 发布的 Flow definition，包含 Queue Channel、Example RPC、WaitForScheduleStep 和 ForceCompleteIfChannelsEmpty"
...
每个语言 tab 先定义完整的消息处理 Step，包括类型名称以及两个 handler；再定义拥有该 Step 和 **exampleRPC** handler 的 Flow。Step 消费 **queueChannel**；Flow-level RPC 只向这个 Channel 发布消息并返回响应。
