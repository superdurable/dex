...
普通 durable wait 应让 **RequestTimeout** 保持为 0。在 admission 边界应显式设置它。只有需要回收被放弃的 accepted handlers，以避免它们逼近 Temporal 的 in-flight Update limit 时，才应设置 **InternalHandlerTimeout**。详情请参阅[选择超时边界](/design-patterns/responsive-update#选择超时边界)。
