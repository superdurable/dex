...
常规使用应让 **MaximumWaitTime** 保持为 0，并用调用方 deadline 限制单次响应时间。正值极少使用，因为较短的 budget 可能向 Workflow history 增加大量 Temporal Update events。确实需要正值时，建议至少 1 分钟。它会向调用方返回有类型的 timeout。详情请参阅[选择等待 budget](/design-patterns/responsive-update#选择等待-budget)。
