...
普通 durable wait 应让 **RequestTimeout** 保持为 0。在 admission 边界应显式设置它。下面的 runnable controller 暂时保持无限等待，直到它依赖的已发布 SDK 包含这个 API。只有需要回收被放弃的 accepted handlers，以避免它们逼近 Temporal 的 in-flight Update limit 时，才应设置 **InternalHandlerTimeout**。详情请参阅[选择超时边界](/design-patterns/responsive-update#选择超时边界)。
