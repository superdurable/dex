...
普通 durable wait 应让 **RequestTimeout** 保持为 0。在 admission 边界应显式设置它。下面的 runnable controller 暂时保持无限等待，直到它依赖的已发布 SDK 包含这个 API。**InternalHandlerTimeout** 是高级 Update generation 控制项，通常保持为 0。详情请参阅[选择超时边界](/design-patterns/responsive-update#选择超时边界)。
