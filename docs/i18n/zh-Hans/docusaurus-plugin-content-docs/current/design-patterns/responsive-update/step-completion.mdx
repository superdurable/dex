...
Dex 会从 Step execution 派生 Request ID，因此等待同一里程碑的调用方会共享一个已接受的 durable Update。普通的无限 handler 等待应让 **MaximumWaitTime** 保持为 0。本地 HTTP 或 client deadline 仍可限制一次响应的时长，而不会取消该已接受的 handler。只有当一个 Flow 可能积累被放弃或很少完成的 Step 等待时，才设置正值。过期会释放 in-flight slot，但继续等待会创建新的 -N Update generation。容量和成本的取舍请参阅[选择 durable handler 的生命周期](/design-patterns/responsive-update#选择-durable-handler-的生命周期)。
