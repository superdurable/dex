...
用 [为什么是 Dex？](/intro/what-is-dex) 里同一条 order-processing Flow，从头跑一遍：安装 Dex，注册 Worker 和 Client，用 HTTP controller 启动 Flow，批准发货，再在 Dex Web 里看 Step graph。

SDK 选一次即可。偏好存在这个浏览器（`localStorage`），默认是 **Python**。

可运行的样例在各语言的 `examples/` 目录，见下面 tab 的链接。样例带 HTTP 路由；也可以用 examples playground（`examples/playground`，`id: order-processing`）。见 `examples/README.md`。
...
同一台机器上再开一个 `dexcli dev` 会绑到别的空闲端口，并用自己的 SQLite。把打印出来的 Dex Server 地址传给 `--server`。flag 和持久化选项见 [CLI README](https://github.com/superdurable/dex/blob/main/cli/README.md)。

## 2. Flow、Step、RPC、constructor 依赖

先扣款，再等卖家批准（Channel 或提醒 Timer），然后发货。发货 retry 用尽就退款。设计见 [为什么是 Dex？](/intro/what-is-dex)。
...
把 Registry、Worker、Client 指到 `dexcli`（`127.0.0.1:8801`）。用 service 构造 Flow，再注册。
...
从 JSON 里复制 `flowID`。打开 [http://127.0.0.1:8802](http://127.0.0.1:8802)，找到这条 Flow，打开详情。ShipStep 在等待。

```bash
curl -s "http://127.0.0.1:8080/products/order-processing/approve?workflowId=FLOW_ID"
```

把 `FLOW_ID` 换成 start 的返回值。这条 Flow 会发货并完成。

## 6. Dex Web

start 并 approve 之后，打开这条 Flow。Step graph 里是 Charge 然后 Ship：

![approve 之后的 Dex Web Step graph](/img/quick-start/step-graph.png)

Timeline tab 列出同一次 run：

![approve 之后的 Dex Web Timeline](/img/quick-start/timeline.png)
