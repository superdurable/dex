...
用 [为什么是 Dex？](/intro/what-is-dex) 里同一条 order-processing Flow，从头跑一遍：安装 Dex，注册 Worker 和 Client，用 HTTP controller 启动 Flow，批准发货，再在 Dex Web 里看 Execution graph。
...
![approve 之后的 Dex Web Execution graph](/img/quick-start/step-graph.png)
