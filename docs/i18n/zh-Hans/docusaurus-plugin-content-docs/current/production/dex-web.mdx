...
在 **v2** 中，在画布上选中 Step 会打开该 current run 的详情面板。**Input**、**Output** 和 **Context** 以与 **v1** Selected event 相同的结构化方式展示选中的 WaitFor 或 Execute history 事件。面板默认 Details，可切换为已水合 payload 的 Raw JSON。十进制数字形式的 AttributeMap / ChannelMap instance 按数值排序，行为与 **v1** 一致。Execute 显示该次 run 决策里的 live next Steps，而不是 FDG 上的全部出口。若 Continue-as-New 把该 Step 留在上一 hop，使用 Load more from previous run。
