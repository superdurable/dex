...
Server 会把业务写入覆盖到权威 Attribute state 上，并在同一个 Workflow Task 中计算 mapping。它对当前可用 Action 的 permission 排序、去重，仅在 union 变化时完整替换内部 KeywordList Search Attribute **DexWorkQueuePermissions**；空 union 会删除该投影。维护投影不要求应用 Step 锁住每个 Action source。

Action definition 是 active Flow 的稳定契约。部署时删除或修改全部 mapping，并不会自动迁移已有状态。后续 source write、显式空 mapping 或新的 Flow run 才会应用新定义。

Work Queue 会列出所选 Flow type 声明的 permission。在 **Working as** 中选择一个 permission 后，浏览器通过 **POST /api/v2/search** 的 **workQueuePermissions** 发送它。Superverse 等调用方也可以发送从多个用户 role 得到的多个 permission。只要 run 的投影列表包含任意一个请求 permission 就会匹配；这组 permission 与 Flow type 和其他 filter 之间使用 AND。

投影匹配是必要条件，而非充分条件。带 key 的 gate 还需要它当前的 key，而列表读取的是最终一致的索引。真正可用的 Action 仍由打开后的该 run 决定。
