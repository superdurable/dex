...
每个 Action 要求一个 permission，例如 **refund.manage**。Permission key 以小写字母开头，只包含小写字母、数字、点或连字符。Go Worker 会在相关状态变化后计算所有 Action condition，并把当前可用 Action 的 permission 去重后投影到内部 KeywordList Search Attribute **DexWorkQueuePermissions**。
