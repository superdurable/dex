...
下面的标签页展示 Flow 拓扑、重试策略和补偿路径。每个链接指向可运行的完整示例，其中还包含请求模型、依赖服务、HTTP controller 和集成测试。
...
启动 Dex 和任意语言的示例服务，然后通过 [examples playground](https://github.com/superdurable/dex/tree/main/examples/playground) 启动 Money transfer。下面是一次已完成的 Go 运行：它把 42 个单位从 **from-ci** 转到 **to-ci**，每个业务操作都显示为独立且已完成的 Step。
