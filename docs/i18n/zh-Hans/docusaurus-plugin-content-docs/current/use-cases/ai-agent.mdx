...
普通 user message 会进入 durable **UserMessages** FIFO queue。Agent loop 运行期间，这些 message 会保持 pending，不会中断当前 model 或 tool call。UI 会立即显示它们。只要 message 尚未被消费，用户就能 Edit、Delete 或选择 Steer。Edit 会先删除 pending message；重新提交时，它会带着新的 ID 加到 queue 尾部。

Steer 会在一个 transaction 中删除选中的 regular message，并把相同 value 发布到 **ImmediateUserMessages**。Agent 会在下一次 model call、tool、approval wait 或 Timer continuation 前检查这条 queue。它不会取消已经开始的 LLM 或 MCP request。到达下一个 safe boundary 后，Agent 会清除尚未执行的 tool call 与过期 approval 或 Timer state，记录结构化 cancellation result，再根据 Steer message 重新规划。
...
内置 **durable_wait** tool 会把 Flow 移动到同时等待 [Timer](/primitives/timer) 与 **ImmediateUserMessages** 的 Step。Timer 可以跨 Worker restart 继续等待。普通 message 会继续排队；选择 Steer 才会中断等待并让 Agent 重新规划。
...
Active work 期间提交的 message 会以 **Queued** 状态显示在 conversation 与 composer 之间。消费前可以用 **Edit** 或 **Delete**，也可以用 **Steer** 在下一个 safe boundary 让 Agent 处理它。
