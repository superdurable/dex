...
Local playground 会启动无需 credential 的 search、Slack 和 Google Docs demo MCP server。Model call 运行时，chat page 会分别展示 Thinking、Response 与 Agent activity。可以使用 **Command/Ctrl+Enter** 或 **Alt+Enter** 发送 message。当 plan 缺少必要信息时，内置 **request_user_input** tool 会创建 durable question，并等待下一条 user reply 后再继续执行。
