...
Version 2.0 会通过 config 中 canonical Connector SDK marker 与生成的字段 metadata 识别 **openai.NewCreateResponseStep** 等 operation-specific Connector factory，而不维护 connector 或函数名列表。Dex Web 会用 Connector 图标标记这些 Step。通用 SDK 的 **MustNewQueryStep** 和 **MustNewMutationStep** escape hatch 仍然受支持。Factory 调用必须直接位于 **DefineStep** 或 **DefineStartStep** 中，并使用内联 config literal。Step type、presentation、具名 **GoTo** branch target（或通用 **GoToBranch** target）、**StepRef** 名称、Result Attribute、Stream 和 **ProceedToOnExecuteFailure** target 都必须能够静态确定。Factory presentation 直接提供 Step group 和 explanation，因此生成的 Step 不需要本地 directive。定义图会为每个 Connector branch 生成一条带标签的 transition，为 Result Attribute 生成 write relation，并为 structured/text Stream 生成 best-effort progress relation。动态 config、branch slice、target 或 resource reference 会产生 blocking diagnostic。参见 [Connector factory 示例](https://github.com/superdurable/dex/blob/main/examples/go/products/connector-factory/workflow.go)。
