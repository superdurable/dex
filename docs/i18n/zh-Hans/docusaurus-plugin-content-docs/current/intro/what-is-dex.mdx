...
Dex 是 **Durable Execution** 平台。

Dex 是结构化编程，只有少数几个概念作为 durable primitive。你用 Dex 写 **Flow**，里面填普通代码：durable 的 **Step**、**Attribute**、**RPC**，以及用 **Channel** 和 **Timer** 表达的 durable condition。然后跑托管 Flow 的 **Worker**。**Client** 调 **Dex Server** 去 start 并和 Flow 实例交互。Dex Server 把 Step / RPC 调用任务派发给你的 Worker。
...
还没看过 [什么是 Durable Execution？](/intro/what-is-durable-execution) 的话，这里用那里的 order-processing 例子。
...
HTTP 前缀 `/products/order-processing/...`。用 [examples playground](https://github.com/superdurable/dex/tree/main/examples/playground) 试（`id: order-processing`）。Rust：[`examples/rust/src/products/order_processing`](https://github.com/superdurable/dex/tree/main/examples/rust/src/products/order_processing)。
