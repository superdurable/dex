...
| Flow 在 regular Activity 完成前关闭 | Activity scheduled/started + Flow closed | `FlowClosedHistoryEvent.pending_step_methods` 保留未完成 method |

`durability` 表示请求的 Dex durability。ASYNC fallback 最终由 regular Activity 完成时仍返回 `STEP_DURABILITY_ASYNC`。

Pending method 保留统一的 `input/context`，并以 `SCHEDULED` 或 `STARTED`
标识关闭前最后一个已持久化的 Activity lifecycle 阶段。
