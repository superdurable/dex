...
* Meaning: The State API is failing. The API is implemented by the Dex application. 
* Investigation:
  * Look at logs to see why the State API is failing.
  * If there are no error logs, then check if there is some devops/network issue between Dex server and application 
...
Dex Application is REST based micro-service. To scale it up, you simply add instances. 

## Make a better view of workflow in Temporal UI
To make it easier with Dex, customize the columns by clicking the button from sidebar, to replace WorkflowType with DexWorkflowType
...
### !!!Important Tips!!!
* Let your worker service return error stacktrace as the response body to Dex server. E.g.
  like [this example of Spring Boot using ExceptionHandler](../../examples/java/src/main/java/io/dex/controller/DexWorkerApiController.java#L51)
...
**These are ONLY events that you need to understand. You can ignore the others as implementation details of Dex service**
...
If the signal name starts with __DexSystem_ .. then it's a system signal
They are for internal implementation of Dex service. e.g. SkipTimerChannel is for skipping a timer.
...
On top of the Temporal system search attributes, Dex also provide three system search attributes:

* `DexWorkflowType`: Keyword:  
* `DexExecutingStateIds`: Keyword array
* `DexGlobalWorkflowVersion`: Int

For example, you can search for workflow type that is executing a state: `DexWorkflowType='AbcWorkflow' AND DexExecutingStateIds='StateA'`  
...
You can also use your [custom search attributes in Dex](Persistence.md):
...
With Dex it's recommended to reset through the Dex service API.
...
* STATE_EXECUTION_ID: reset the the particular execution of the state. Dex use increasing number from 1,2,3... to order the state execution for the same StateId. E.g. WaitAndPrepareState-2 means the Id of the second time that state is executed. must be used with stateExecutionId 
...
`curl -X POST https://dex-service.com/api/v1/workflow/reset -d '{ "workflowId": "<workflowId>", "resetType": "STATE_ID", "stateId": "WaitAndPrepareState", "skipSignalReapply": true }'`


See Dex service [`dex.proto`](../../protos/dex.proto) for more details.

## How To Skip Timer for Workflows
Any the timer commands can be skipped by an API call. The API is provided by Dex server for free. This can be used for operation or testing purpose. 
...
`curl -X POST https://dex-service.com/api/v1/workflow/timer/skip -d '{ "workflowId": "<workflowId>", "workflowStateExecutionId": "WaitAndPrepareState-1", "timerCommandIndex": 0 }'`
...
**Dex requires a format for the signalValue. Make sure you test it in QA first!**
...
Alternatively, use this CURL command to call Dex service:

`curl -X POST https://dex-service.com/api/v1/workflow/signal -d '{ "workflowId": "<workflowId>", "signalChannelName": "MySignalName", "signalValue": { "encoding": "jsonType", "data": "\"a string value\"" } }'`
...
**Dex service is sensitive to the format. Make sure you test it in QA first!**
...
## More Operations
All other operation that you can do is defined in the [`dex.proto`](../../protos/dex.proto) of Dex service. All the operations supported in SDKs can be done using CURL command:
