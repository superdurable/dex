...
import { leadFields, runRow, uiSlotsOf } from './uiSlots';

const field = (attributeKey: string, uiSlot?: string) => ({
  attributeKey, valueType: 'string', editable: false, description: attributeKey, uiSlot,
...
describe('uiSlotsOf', () => {
  it('groups declared fields by slot, in declaration order', () => {
    const uiSlots = uiSlotsOf(refund);
    expect(uiSlots.title?.map((f) => f.attributeKey)).toEqual(['customer-email']);
    expect(uiSlots.reason?.map((f) => f.attributeKey))
      .toEqual(['recommendation-rationale', 'guardrail-rule']);
  });

  it('ignores a field with no slot, so the detail list keeps it', () => {
    expect(Object.values(uiSlotsOf(refund)).flat().map((f) => f?.attributeKey))
      .not.toContain('operator-note');
  });

  it('ignores a slot name the UI does not render rather than trusting the file', () => {
    expect(uiSlotsOf(definition([field('x', 'headline')]))).toEqual({});
  });

  it('is empty for a Flow that declares nothing', () => {
    expect(uiSlotsOf(undefined)).toEqual({});
    expect(uiSlotsOf(definition([]))).toEqual({});
  });
});

describe('runRow', () => {
  it('names the run with what the Flow declared, not the Flow ID', () => {
    const row = runRow(flow(), uiSlotsOf(refund));
  ...
  it('falls back to the Flow ID and execution status when no slot is declared', () => {
    const row = runRow(flow(), uiSlotsOf(definition([])));
  ...
  it('falls back when a slot is declared but this run carries no value for it', () => {
    const row = runRow(flow({ indexedAttributes: {}, summary: {} }), uiSlotsOf(refund));
    expect(row.title).toBe('adv-1');
    expect(row.status).toBe('Running');
  });

  it('treats an empty string as no value, not as a title', () => {
    const row = runRow(flow({ indexedAttributes: { 'customer-email': '' } }), uiSlotsOf(refund));
    expect(row.title).toBe('adv-1');
  });

  it('reads a slot out of Summary when it is not indexed', () => {
    const summaryTitle = definition([field('recommended-action', 'title')]);
    expect(runRow(flow(), uiSlotsOf(summaryTitle)).title).toBe('IssueRefund');
  });
  ...
  it('renders a number-valued slot as text', () => {
    ...
    expect(runRow(flow({ indexedAttributes: { 'refund-amount': 17500 } }), uiSlotsOf(amount)).status)
      .toBe('17500');
  });
});
