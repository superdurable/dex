...
Install the Git hooks once per clone with `make githooks`. They reject Cursor
co-author trailers and block pushes when a branch does not contain the latest
`origin/main`. Do not pass `--no-verify`.
