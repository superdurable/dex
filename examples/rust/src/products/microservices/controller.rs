...
use crate::products::microservices::flow::OrchestrationFlow;
...
async fn start(State(client): State<SharedClient>, Query(query): Query<StartQuery>) -> impl IntoResponse {
    let flow_id = if query.workflow_id.is_empty() { new_flow_id("microservice") } else { query.workflow_id };
    match run_blocking(move || {
        let flow = OrchestrationFlow::default();
        let input = "workflow".to_string();
