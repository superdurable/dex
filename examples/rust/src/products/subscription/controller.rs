...
async fn start(State(client): State<SharedClient>, Query(query): Query<StartQuery>) -> impl IntoResponse {
    let flow_id = if query.workflow_id.is_empty() { new_flow_id("subscription") } else { query.workflow_id };
    match run_blocking(move || {
        let flow = SubscriptionFlow::default();
        let input = SubscriptionRequest { customer_id: "customer-1".into(), charge_cents: 1000, billing_periods: 3 };
