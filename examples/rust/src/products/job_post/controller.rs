...
use crate::products::job_post::flow::{JobPost, JobPostFlow};
...
async fn start(State(client): State<SharedClient>, Query(query): Query<StartQuery>) -> impl IntoResponse {
    let flow_id = if query.workflow_id.is_empty() { new_flow_id("job-post") } else { query.workflow_id };
    match run_blocking(move || {
        let flow = JobPostFlow::default();
        let input = JobPost { title: "Engineer".into(), description: "Build flows".into(), ..JobPost::default() };
