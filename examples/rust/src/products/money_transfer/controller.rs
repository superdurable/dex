...
use crate::products::money_transfer::flow::MoneyTransferFlow;
...
pub fn mount(client: SharedClient) -> Router {
    Router::new()
        .route("/products/money-transfer/start", get(start))
        .with_state(client)
}

async fn start(State(client): State<SharedClient>, Query(query): Query<StartQuery>) -> impl IntoResponse {
    let flow_id = if query.workflow_id.is_empty() { new_flow_id("money-transfer") } else { query.workflow_id };
    match run_blocking(move || {
        let flow = MoneyTransferFlow::default();
        let input = crate::products::money_transfer::flow::TransferRequest { from_account: "a".into(), to_account: "b".into(), amount_cents: 1000, notes: "notes".into() };
