...
use crate::patterns::scalable_parallel::flow::RequestReceiverFlow;
...
pub fn mount(client: SharedClient) -> Router {
    Router::new()
        .route("/patterns/scalable-parallel/start", get(start))
        .with_state(client)
}

async fn start(State(client): State<SharedClient>, Query(query): Query<StartQuery>) -> impl IntoResponse {
    let flow_id = if query.workflow_id.is_empty() { new_flow_id("scalable") } else { query.workflow_id };
    match run_blocking(move || {
        let flow = RequestReceiverFlow::default();
        let input = ();
