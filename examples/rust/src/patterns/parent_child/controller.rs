...
use crate::patterns::parent_child::flow::ParentFlowV2;
...
pub fn mount(client: SharedClient) -> Router {
    Router::new()
        .route("/patterns/parent-child/start", get(start))
        .with_state(client)
}

async fn start(State(client): State<SharedClient>, Query(query): Query<StartQuery>) -> impl IntoResponse {
    let flow_id = if query.workflow_id.is_empty() { new_flow_id("parent") } else { query.workflow_id };
    match run_blocking(move || {
        let flow = ParentFlowV2::default();
        let input = "child-task".to_string();
