...
use crate::patterns::interruptible::flow::InterruptibleExecutionFlow;
...
async fn start(State(client): State<SharedClient>, Query(query): Query<StartQuery>) -> impl IntoResponse {
    let flow_id = if query.workflow_id.is_empty() { new_flow_id("interrupt") } else { query.workflow_id };
