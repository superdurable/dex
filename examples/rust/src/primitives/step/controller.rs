...
pub fn mount(client: SharedClient) -> Router {
    Router::new()
        .route("/primitives/step/start", get(start))
        .route("/primitives/step/retry/start", get(retry_start))
        .with_state(client)
}

#[derive(Deserialize)]
struct RetryStartQuery {
    #[serde(default, rename = "workflowId")]
    workflow_id: String,
    #[serde(default, rename = "readyAfterAttempt")]
    ready_after_attempt: i32,
}

async fn retry_start(
    State(client): State<SharedClient>,
    Query(query): Query<RetryStartQuery>,
) -> impl IntoResponse {
    let flow_id = query.workflow_id;
    let ready_after_attempt = query.ready_after_attempt;
    match run_blocking(move || {
        let flow = RetryFlow::default();
        client
            .start_flow(&flow, &flow_id, ready_after_attempt)
