...
use crate::primitives::heartbeat::flow::HeartbeatFlow;
...
pub fn mount(client: SharedClient) -> Router {
    Router::new()
        .route("/primitives/step/heartbeat/start", get(start))
        .with_state(client)
}
...
async fn start(
    ...
    let batches = query.batches;
    match run_blocking(move || {
        let flow = HeartbeatFlow::default();
        client
            .start_flow(&flow, &flow_id, batches)
