...
import { rpcFlow } from "./rpc-flow.js";

export function createRpcRouter(client: Client): Router {
  const router = Router();

  router.get("/start", async (request, response) => {
    const workflowId = String(request.query.workflowId ?? "");
    const runId = await client.startFlow(rpcFlow, workflowId, 0, startOptions());
    response.json({ flowID: workflowId, runID: runId });
  });

  router.get("/trigger", async (request, response) => {
    const workflowId = String(request.query.workflowId ?? "");
