...
import { durabilityFlow } from "./durability-flow.js";

export function createDurabilityRouter(client: Client): Router {
  const router = Router();
  ...
  router.get("/start", async (request, response) => {
    ...
    const runId = await client.startFlow(durabilityFlow, workflowId, mode, startOptions());
