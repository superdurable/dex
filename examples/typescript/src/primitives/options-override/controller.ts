...
import { optionsOverrideFlow } from "./options-override-flow.js";

export function createOptionsOverrideRouter(client: Client): Router {
  const router = Router();
  ...
  router.get("/start", async (request, response) => {
    ...
    const runId = await client.startFlow(optionsOverrideFlow, workflowId, input, startOptions());
