...
import { stepDecisionFlow } from "./step-decision-flow.js";

export function createStepDecisionRouter(client: Client): Router {
  const router = Router();
  ...
  router.get("/start", async (request, response) => {
    ...
    const runId = await client.startFlow(stepDecisionFlow, workflowId, mode, startOptions());
