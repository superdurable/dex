...
import { subFlowParentFlow } from "./subflow-flow.js";

export function createSubflowRouter(client: Client): Router {
  const router = Router();
  ...
  router.get("/start", async (request, response) => {
    ...
    const runId = await client.startFlow(subFlowParentFlow, workflowId, inputNum, startOptions());
