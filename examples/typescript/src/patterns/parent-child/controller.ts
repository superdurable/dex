...
import { parentFlowV2 } from "./parent-flow-v2.js";

export function createParentChildRouter(client: Client): Router {
  const router = Router();
  ...
  router.get("/start", async (request, response) => {
    ...
    await client.startFlow(parentFlowV2, workflowId, numOfChildWfs, {
      ...startOptions(),
