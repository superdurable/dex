...
the difference between the Dex State API Backoff Retry and the Failure Recovery
feature. These two are independent, but strongly related. The Recovery Feature
is used when the Dex State API Backoff Retry attempts exhausted. The key concept
is to let state API retry for a number of times, and if finally failed, then go
to the recovery solution.

## Dex State API Backoff Retry and Failure Handling After Retry Exhausted

Dex State API (waitUntil/execute) will retry on any failure. If the retry policy
...
- InitialIntervalSeconds: 1
   - The initial amount time, in seconds, between a state failure and Dex
   retrying a state.
- MaxInternalSeconds: 100
   - The maximum amount of time, in seconds, between a state failure and Dex
   retrying a state.
- MaximumAttempts: 0
   - The maximum number of times Dex will retry a failing state.
   - `0` means infinite
- MaximumAttemptsDurationSeconds: 0
   - The maximum total time, in seconds, that Dex will allow a failing state to
   retry.
  - `0` means infinite
...
`MaximumAttempts` tells Dex that, if this state API continues to fail, only
retry the state this number of times. `MaximumAttemptsDurationsSeconds` tells Dex
...
![Failure Recovery Workflow Diagram](./assets/Dex-Design-Patterns_FailureRecoveryWorkflow.png)
...
- [Dex Workflow Options Wiki](https://github.com/superdurable/dex/wiki/WorkflowStateOptions)
