...
curl -X POST http://localhost:8080/design-pattern/entity-store/profile \
  ...
  -d '{"userId":"user-42","displayName":"Ada Lovelace","email":"ada@example.com","marketingOptIn":true}'

curl -X POST http://localhost:8080/design-pattern/entity-store/profile/update \
  -H 'content-type: application/json' \
  -d '{"userId":"user-42","displayName":"Ada Byron","email":"ada.byron@example.com","marketingOptIn":false}'
```
...
The row remains and its three projected columns become SQL `NULL`. A projection
