...
import static io.superdurable.dex.patterns.workflow.recovery.FailureRecoveryWorkflow.WORKFLOW_INPUT_KEY;
...
class UpdateItemQuantityState implements WorkflowState<FailureRecoveryWorkflowInput> {
    ...
    public WorkflowStateOptions getStateOptions() {
        ...
        // the only way to recover from that state is to set the retry policy to tell Dex how to handle a state that will not
        // succeed.
        //
        // The main controls used for workflow recovery are the `ProceedToStateWhenExecuteRetryExhausted` and
        // `ExecuteApiRetryPolicy`. `ProceedToStateWhenExecuteRetryExhausted` tells Dex which state to move to when the current
        // state has failed. `ExecuteApiRetryPolicy` tells Dex how long or how many times a state is able to continue to
        ...
        return new WorkflowStateOptions()
                ...
                // a retry policy that tells Dex when to allow the state to fail.
                .setProceedToStateWhenExecuteRetryExhausted(UpdateQuantityRecoveryState.class)
                // `ExecuteApiRetryPolicy` tells Dex for how long or how many times a state is allowed to fail and retry. Here,
