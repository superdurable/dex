...
- **Manual Intervention**: If human interaction is need or more data is needed, a user can send a signal with or without data through the Temporal Cloud UI (internal channels and external signal channels supported) , using DEX's built-in endpoint (only external signal channels supported), or custom rpc endpoints defined in the workflow (internal channels and external signal channel supported).
## Workflow Details

![Manual Intervention Workflow](./assets/DEX-design-patters_manual-intervention.png)
