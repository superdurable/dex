...
func (store *localFileStoreImpl) scan() (scanResult, error) {
	...
	if err := filepath.WalkDir(store.blobsDir, collector.visit); err != nil {
...
func (store *localFileStoreImpl) readOpenFile(file *os.File, entry *diskEntry) ([]byte, error) {
	...
	if blobID != entry.blobID || header.checksum != entry.checksum || fileInfo.Size() != entry.size {
...
func (store *localFileStoreImpl) readPrefix(
	file *os.File,
	fileSize int64,
	path string,
) (fileHeader, string, error) {
