...
// ValidateAndSortStateSelections validates and sorts collection names and instances.
func ValidateAndSortStateSelections(
	attributeMapInstances []string,
	channelNames []string,
	channelMapInstances []string,
...
func validateAndSortMapInstances(kind string, instances []string) ([]string, error) {
	...
	for _, instance := range instances {
		if strings.TrimSpace(instance) == "" {
			return nil, fmt.Errorf("state load %s instance is empty", kind)
		}
		separatorIndex := strings.IndexByte(instance, '/')
		if separatorIndex <= 0 || separatorIndex != strings.LastIndexByte(instance, '/') {
			return nil, fmt.Errorf(
				"state load %s instance %q must contain one '/' after the definition name",
				kind,
				instance,
			)
		}
		if _, exists := seen[instance]; exists {
			return nil, fmt.Errorf("state load %s instance %q is duplicated", kind, instance)
		}
...
func validateAndSortDefinitionNames(kind string, names []string) ([]string, error) {
	...
	for _, name := range names {
		if strings.TrimSpace(name) == "" {
			return nil, fmt.Errorf("state load %s name is empty", kind)
		}
		if strings.Contains(name, "/") {
			return nil, fmt.Errorf("state load %s name %q contains '/'", kind, name)
		}
		if _, exists := seen[name]; exists {
			return nil, fmt.Errorf("state load %s name %q is duplicated", kind, name)
		}
		seen[name] = struct{}{}
