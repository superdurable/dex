...
import (
	...
	"github.com/xcherryio/xcherry/server/utils"
)
...
func NewServiceImpl(
	...
	membershipImpl := async2.NewMembershipImpl(cfg, logger, nil, async2.ServerTypeApi)
...
func (s serviceImpl) StartProcess(
	...
	storeReq := data_models2.StartProcessRequest{
		Request:        request,
...
func (s serviceImpl) StopProcess(
	...
	resp, err := s.processStore.StopProcess(ctx, data_models2.StopProcessRequest{
...
func (s serviceImpl) DescribeLatestProcess(
	...
	resp, perr := s.processStore.DescribeLatestProcess(ctx, data_models2.DescribeLatestProcessRequest{
		Namespace: request.Namespace,
		ProcessId: request.ProcessId,
	})
	if perr != nil {
...
func (s serviceImpl) PublishToLocalQueue(
	...
	resp, err := s.processStore.PublishToLocalQueue(ctx, data_models2.PublishToLocalQueueRequest{
		Namespace: request.GetNamespace(),
		ProcessId: request.GetProcessId(),
...
func (s serviceImpl) Rpc(
	...
	latestPrcExe, err := s.processStore.GetLatestProcessExecution(ctx, data_models2.GetLatestProcessExecutionRequest{
		Namespace: request.GetNamespace(),
		ProcessId: request.GetProcessId(),
	})
	if err != nil {
	...
	if latestPrcExe.AppDatabaseConfig != nil {
		appDatabaseReadResp, err := s.processStore.ReadAppDatabase(ctx, data_models2.AppDatabaseReadRequest{
	...
	updateResp, err := s.processStore.UpdateProcessExecutionForRpc(ctx, data_models2.UpdateProcessExecutionForRpcRequest{
		Namespace:          request.Namespace,
		ProcessId:          request.ProcessId,
		ProcessType:        latestPrcExe.ProcessType,
...
func (s serviceImpl) WaitForProcessCompletion(
	...
	if latestPrcExe.Status != data_models2.ProcessExecutionStatusUndefined &&
		latestPrcExe.Status != data_models2.ProcessExecutionStatusRunning {
		return &xcapi.ProcessExecutionWaitForCompletionResponse{
