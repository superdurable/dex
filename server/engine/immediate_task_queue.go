...
type immediateTaskQueueImpl struct {
	...
	tasksToCommitChan chan data_models2.ImmediateTask
...
func NewImmediateTaskQueueImpl(
	...
	return &immediateTaskQueueImpl{
		...
		processor:                 processor,
		tasksToCommitChan:         make(chan data_models2.ImmediateTask, qCfg.ProcessorBufferSize),
		currentReadCursor:         0,
...
func (w *immediateTaskQueueImpl) pollAndDispatchAndPrepareNext() {
	...
	resp, err := w.store.GetImmediateTasks(
		w.rootCtx, data_models2.GetImmediateTasksRequest{
			ShardId:                w.shardId,
...
func (w *immediateTaskQueueImpl) receiveCompletedTask(task data_models2.ImmediateTask) {
