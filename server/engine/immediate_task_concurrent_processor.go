...
type immediateTaskConcurrentProcessor struct {
	...
	cfg               config.Config
	taskToProcessChan chan data_models2.ImmediateTask
	// shardId: channel
	taskToCommitChans map[int32]chan<- data_models2.ImmediateTask
...
func NewImmediateTaskConcurrentProcessor(
	...
	return &immediateTaskConcurrentProcessor{
		rootCtx:           ctx,
		cfg:               cfg,
		taskToProcessChan: make(chan data_models2.ImmediateTask, bufferSize),
		taskToCommitChans: make(map[int32]chan<- data_models2.ImmediateTask),
...
func (w *immediateTaskConcurrentProcessor) GetTasksToProcessChan() chan<- data_models2.ImmediateTask {
	return w.taskToProcessChan
}

func (w *immediateTaskConcurrentProcessor) AddImmediateTaskQueue(
	shardId int32, tasksToCommitChan chan<- data_models2.ImmediateTask,
) (alreadyExisted bool) {
	w.lock.Lock()
	defer w.lock.Unlock()
...
func (w *immediateTaskConcurrentProcessor) processImmediateTask(
	ctx context.Context, task data_models2.ImmediateTask,
) error {

	w.logger.Debug("start executing immediate task", tag.ID(task.GetTaskId()), tag.ImmediateTaskType(task.TaskType.String()))

	if task.TaskType == data_models2.ImmediateTaskTypeNewLocalQueueMessages {
		return w.processLocalQueueMessagesTask(ctx, task)
	} else if task.TaskType == data_models2.ImmediateTaskTypeVisibility {
		return w.processVisibilityTask(ctx, task)
	}

	prep, err := w.processStore.PrepareStateExecution(ctx, data_models2.PrepareStateExecutionRequest{
	...
	if prep.Status == data_models2.StateExecutionStatusWaitUntilRunning {
		return w.processWaitUntilTask(ctx, task, *prep, apiClient)
	} else if prep.Status == data_models2.StateExecutionStatusExecuteRunning {
...
func (w *immediateTaskConcurrentProcessor) processVisibilityTask(
	ctx context.Context, task data_models2.ImmediateTask) error {
	if task.ImmediateTaskInfo.VisibilityInfo == nil {
		return fmt.Errorf("visibility info is not set")
	}
	err := w.visibilityStore.RecordProcessExecutionStatus(ctx, data_models2.RecordProcessExecutionStatusRequest{
	...
	return w.processStore.DeleteImmediateTasks(ctx, data_models2.DeleteImmediateTasksRequest{
...
func (w *immediateTaskConcurrentProcessor) processWaitUntilTask(
	ctx context.Context, task data_models2.ImmediateTask,
	prep data_models2.PrepareStateExecutionResponse, apiClient *xcapi.APIClient,
) error {
	...
	compResp, err := w.processStore.ProcessWaitUntilExecution(ctx, data_models2.ProcessWaitUntilExecutionRequest{
		ProcessExecutionId: task.ProcessExecutionId,
		StateExecutionId: data_models2.StateExecutionId{
			StateId:         task.StateId,
			StateIdSequence: task.StateIdSequence,
...
func (w *immediateTaskConcurrentProcessor) applyStateFailureRecoveryPolicy(
	...
	case xcapi.FAIL_PROCESS_ON_STATE_FAILURE:
		resp, errStopProcess := w.processStore.StopProcess(ctx, data_models2.StopProcessRequest{
			Namespace:       prep.Info.Namespace,
			ProcessId:       prep.Info.ProcessId,
	...
	case xcapi.PROCEED_TO_CONFIGURED_STATE:
		...
		err := w.processStore.RecoverFromStateExecutionFailure(ctx, data_models2.RecoverFromStateExecutionFailureRequest{
			Namespace:          prep.Info.Namespace,
			ProcessExecutionId: task.ProcessExecutionId,
			SourceStateExecutionId: data_models2.StateExecutionId{
				StateId:         task.StateId,
				StateIdSequence: task.StateIdSequence,
			},
		...
		nextImmediateTask := data_models2.ImmediateTask{
			ShardId:            task.ShardId,
			ProcessExecutionId: task.ProcessExecutionId,
			StateExecutionId: data_models2.StateExecutionId{
		...
		if proceedStateConfig == nil || !proceedStateConfig.GetSkipWaitUntil() {
			nextImmediateTask.TaskType = data_models2.ImmediateTaskTypeWaitUntil
		} else {
			nextImmediateTask.TaskType = data_models2.ImmediateTaskTypeExecute
		}
...
func createWorkerTaskBackoffInfo() *data_models2.WorkerTaskBackoffInfoJson {
	return &data_models2.WorkerTaskBackoffInfoJson{
...
func (w *immediateTaskConcurrentProcessor) processExecuteTask(
	...
	compResp, err := w.processStore.CompleteExecuteExecution(ctx, data_models2.CompleteExecuteExecutionRequest{
		ProcessExecutionId: task.ProcessExecutionId,
		StateExecutionId: data_models2.StateExecutionId{
			StateId:         task.StateId,
			StateIdSequence: task.StateIdSequence,
	...
	if ok && compResp.ProcessStatus != data_models2.ProcessExecutionStatusUndefined &&
		compResp.ProcessStatus != data_models2.ProcessExecutionStatusRunning {
...
func (w *immediateTaskConcurrentProcessor) createContextWithTimeout(
	ctx context.Context, taskType data_models2.ImmediateTaskType, stateConfig *xcapi.AsyncStateConfig,
) (context.Context, context.CancelFunc) {
	qCfg := w.cfg.AsyncService.ImmediateTaskQueue
	timeout := qCfg.DefaultAsyncStateAPITimeout
	if stateConfig != nil {
		if taskType == data_models2.ImmediateTaskTypeWaitUntil {
		...
		} else if taskType == data_models2.ImmediateTaskTypeExecute {
...
func (w *immediateTaskConcurrentProcessor) notifyNewImmediateTask(
	shardId int32, prep data_models2.PrepareStateExecutionResponse, task data_models2.ImmediateTask,
) {
...
func (w *immediateTaskConcurrentProcessor) checkRetry(
	task data_models2.ImmediateTask, info data_models2.AsyncStateExecutionInfoJson,
) (nextBackoffSeconds int32, shouldRetry bool) {
	if task.TaskType == data_models2.ImmediateTaskTypeWaitUntil {
	...
	} else if task.TaskType == data_models2.ImmediateTaskTypeExecute {
		return GetNextBackoff(
			task.ImmediateTaskInfo.WorkerTaskBackoffInfo.CompletedAttempts,
			task.ImmediateTaskInfo.WorkerTaskBackoffInfo.FirstAttemptTimestampSeconds,
...
func (w *immediateTaskConcurrentProcessor) retryTask(
	ctx context.Context, task data_models2.ImmediateTask,
	prep data_models2.PrepareStateExecutionResponse, nextIntervalSecs int32,
	LastFailureStatus int32, LastFailureDetails string,
) error {
	fireTimeUnixSeconds := time.Now().Unix() + int64(nextIntervalSecs)
	err := w.processStore.BackoffImmediateTask(ctx, data_models2.BackoffImmediateTaskRequest{
...
func (w *immediateTaskConcurrentProcessor) composeHttpError(
	...
	info data_models2.AsyncStateExecutionInfoJson, task data_models2.ImmediateTask,
) (int32, string) {
	responseBody := "None"
...
func (w *immediateTaskConcurrentProcessor) processLocalQueueMessagesTask(
	...
	resp, err := w.processStore.ProcessLocalQueueMessages(ctx, data_models2.ProcessLocalQueueMessagesRequest{
		TaskShardId:        task.ShardId,
		TaskSequence:       task.GetTaskSequence(),
		ProcessExecutionId: task.ProcessExecutionId,
...
func (w *immediateTaskConcurrentProcessor) readAppDatabaseIfNeeded(
	ctx context.Context, prep data_models2.PrepareStateExecutionResponse, task data_models2.ImmediateTask,
) (*data_models2.AppDatabaseReadResponse, error) {
	if prep.Info.StateConfig == nil ||
		prep.Info.StateConfig.AppDatabaseReadRequest == nil {
		return &data_models2.AppDatabaseReadResponse{}, nil
	}

	if prep.Info.AppDatabaseConfig == nil {
		return &data_models2.AppDatabaseReadResponse{},
	...
	resp, err := w.processStore.ReadAppDatabase(ctx, data_models2.AppDatabaseReadRequest{
		AppDatabaseConfig: *prep.Info.AppDatabaseConfig,
...
func (w *immediateTaskConcurrentProcessor) loadLocalAttributesIfNeeded(
	...
	if prep.Info.StateConfig == nil ||
		...
		return &data_models2.LoadLocalAttributesResponse{}, nil
	}
	...
	resp, err := w.processStore.LoadLocalAttributes(ctx, data_models2.LoadLocalAttributesRequest{
		ProcessExecutionId: task.ProcessExecutionId,
