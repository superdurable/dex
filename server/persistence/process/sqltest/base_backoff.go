...
func SQLBackoffTest(t *testing.T, ass *assert.Assertions, store persistence.ProcessStore) {
	...
	verifyTimerTask(ass, timerTasks1[0], data_models2.TimerTaskTypeWorkerTaskBackoff, stateId1+"-1",
		data_models2.TimerTaskInfoJson{
			WorkerTaskBackoffInfo: backoffInfo1,
			WorkerTaskType:        ptr.Any(data_models2.ImmediateTaskTypeWaitUntil)})
	...
	verifyTimerTask(ass, timerTasks2[0], data_models2.TimerTaskTypeWorkerTaskBackoff, stateId1+"-1",
		data_models2.TimerTaskInfoJson{
			WorkerTaskBackoffInfo: backoffInfo2,
			WorkerTaskType:        ptr.Any(data_models2.ImmediateTaskTypeWaitUntil)})

	resp, err := store.ConvertTimerTaskToImmediateTask(ctx, data_models2.ProcessTimerTaskRequest{
		Task: timerTasks1[0],
	})
	ass.Nil(err)
	ass.Equal(&data_models2.ProcessTimerTaskResponse{
		HasNewImmediateTask: true,
	}, resp)

	_, _, immediateTasks := checkAndGetImmediateTasks(ctx, t, ass, store, 1)
	verifyImmediateTask(ass, immediateTasks[0], data_models2.ImmediateTaskTypeWaitUntil, stateId1+"-1", data_models2.ImmediateTaskInfoJson{
		WorkerTaskBackoffInfo: &data_models2.WorkerTaskBackoffInfoJson{
			CompletedAttempts:            1,
...
func startProcessAndBackoffWorkerTask(
...
) (int64, *data_models2.WorkerTaskBackoffInfoJson) {
	ctx := context.Background()
	...
	verifyImmediateTaskNoInfo(ass, immediateTask, data_models2.ImmediateTaskTypeWaitUntil, stateId1+"-1")
	...
	backoffInfo := &data_models2.WorkerTaskBackoffInfoJson{
	...
	err := store.BackoffImmediateTask(ctx, data_models2.BackoffImmediateTaskRequest{
