...
func TestStateExecutionLocalQueuesAddMessageAndTryConsume(t *testing.T) {
	...
	stateExecutionLocalQueues := data_models2.NewStateExecutionLocalQueues()
	...
	completedStateExecutionIdString, idx, consumedMessages := stateExecutionLocalQueues.AddMessageAndTryConsume(data_models2.LocalQueueMessageInfoJson{
	...
	assert.Equal(t, []data_models2.InternalLocalQueueMessage{{
...
func TestStateExecutionLocalQueuesTryConsumeForStateExecution_All_consumed(t *testing.T) {
	...
	stateExecutionLocalQueues.AddNewLocalQueueCommands(data_models2.StateExecutionId{
	...
	consumedMessages := stateExecutionLocalQueues.TryConsumeForStateExecution(data_models2.StateExecutionId{
		StateId: "state_1", StateIdSequence: 1,
	...
	assert.Equal(t, map[int][]data_models2.InternalLocalQueueMessage{
		0: {{
			DedupId: uuid_q1_1.String(), IsFull: false,
		}},
		1: {{
...
func TestStateExecutionLocalQueuesTryConsumeForStateExecution_Any_notConsumed(t *testing.T) {
	...
	consumedMessagesMap := stateExecutionLocalQueues.TryConsumeForStateExecution(data_models2.StateExecutionId{
		StateId: "state_1", StateIdSequence: 1,
	}, xcapi.ANY_OF_COMPLETION)
...
//	state_1, 2: (q2, 3),
//	state_3, 1: (q1: 1), (q2, 2)
func prepareDataForAddMessageAndTryConsume(stateExecutionLocalQueues data_models2.StateExecutionLocalQueuesJson) {
	stateExecutionLocalQueues.AddNewLocalQueueCommands(data_models2.StateExecutionId{
		StateId: "state_1", StateIdSequence: 1,
	}, []xcapi.LocalQueueCommand{
...
func prepareDataForTryConsumeForStateExecution(
	stateExecutionLocalQueues data_models2.StateExecutionLocalQueuesJson, dedupIds []uuid.UUID,
) {
	stateExecutionLocalQueues.AddMessageAndTryConsume(data_models2.LocalQueueMessageInfoJson{
