...
`Config.Logger` defaults to `slog.Default` and is available through
`Cache.Logger` for callers that need consistent component logging.
...
The cache does not call `LoadBlobs` or require a Dex server or Temporal.
